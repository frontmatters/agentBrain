...
```
                 ...
                 │ system/+vault│   (public framework + your own vault)
                 └──────┬───────┘
        ┌───────────┬───┴───────┬───────────┐
        ▼           ▼           ▼           ▼
...
doctor, re-wires every agent (`brain wire`), and can roll back. `selftest`
verifies the integration per detected agent, and `uninstall.sh` is symmetric:
it removes exactly what setup added and proves your data untouched.

Day-2 life runs through one CLI — installed as `brain` (and the `agentbrain`
...
- **Writes:** your private vault (`vault/` — a link to a directory outside the
  checkout, `~/.agentBrain/vault` by default; pass `--vault` to choose another
  location); per-agent skills
...
| `templates/vault/`                | Seeds for the vault (READMEs, registry, preference templates); rendered into `vault/` once by setup |
...
| `vault/`                          | **Your vault** — gitignored, never pushed (see below)                   |

### Your vault — mounted at `vault/`

`local/` was the older name of this link. An install from before the rename keeps working; `scripts/setup/drop-local-alias.sh` retires the alias once the agents' own links no longer run through it.

Created automatically by `setup.sh` (via `scripts/setup/setup-structure.sh`) on every machine;
`scripts/installer/bootstrap/macos.sh` is the macOS wrapper that runs it:
...
`vault/spaces/<slug>/` with its own passport (`owner`, a stable `space-id`,
...
| Agent               | Setup                                                                                                           |
| ------------------- | --------------------------------------------------------------------------------------------------------------- |
| **Pi**              | `scripts/installer/bootstrap/macos.sh` — installs Pi, symlinks extensions + skills, generates tsconfig                    |
...
| **Hermes**          | `scripts/setup/setup.sh` — pointer block in `~/.hermes/SOUL.md`                                                        |
...
| `/save-learning`     | Capture a real insight into `vault/learnings/`                   |
...
Your own tool and service notes go in your vault, where agents find
...
| `claude-memory-redirect` | Routes Claude Code's project-memory dir to `vault/memories/projects/<slug>/` via symlink (or sync-hook / instruction-only) |
...
| `session-journal`        | Auto-fills `vault/sessions/session-journal.md` via Stop + PostToolUse hooks; `/journal` slash command |
...
in `vault/learnings/`, a project milestone updates that project's note, and a
...
`git pull` updates the framework without touching `vault/`.
...
### Move — `scripts/sync/move-agentbrain.sh` or `scripts/setup/setup.sh --move-to`
...
```bash
scripts/setup/setup.sh --move-to /Volumes/SSD2/agentBrain
```

### Offboard — `scripts/offboard.sh`

Export knowledge and personal preferences for transfer to another machine. Includes `vault/preferences/personal/`, projects, learnings, daily notes, sessions, reports. Runs privacy scan on export. Team preferences are included only with `--include-team`.
...
### Daily note — `scripts/flow/ensure-daily-note.sh`
