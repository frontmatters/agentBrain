...
## v1.6.3 — Consumer-layout fixes

**The headline:** v1.6.2 installed on a stock single-checkout Linux host (a
Raspberry Pi) surfaced three consumer-layout bugs — all fixed and verified on
that layout.

### Fixes

- **Seal-breach detection on nested vaults**: the space boundary guard now
  catches force-added `spaces/` paths when `local/` lives inside the framework
  checkout (it silently missed them there before).
- **Self-update on consumer installs**: `brain update` and the channel
  resolver now treat a real `~/agentBrain` checkout as the repo instead of
  assuming the maintainer's `~/Developer/agentBrain-dev` path.
- **`brain-extract` names a missing `yq`** instead of failing with a
  misleading "no space-id" error.
