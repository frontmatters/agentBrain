...
Store those in `vault/security/` (gitignored) or your secrets helper / keychain.
...
| Hostnames, IPs           | `mymachine.local`, `192.168.x.x`    | `vault/security/`         |
| Machine-specific notes   | keychain setup, auth migration      | `vault/setup-history/`    |
| Private URLs             | internal Gitea, private repos       | `vault/integrations/`     |
| Credential rotation logs | "rotated X on date Y"               | `vault/security/`         |
| Private infrastructure   | Docker hosts, DB connection strings | `vault/integrations/`     |
...
```
vault/security/privacy-denylist.txt
```
...
Credentials are stored in the macOS keychain and accessed via `~/bin/gitea-helper.sh` (or equivalent secrets helper). See `vault/integrations/` for integration-specific notes.
