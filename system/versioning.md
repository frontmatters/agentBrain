...
1. Fix internal paths in the moved artifact (`vault/...` → `system/...`).
...
4. Commit both sides with conventional messages: the public repo (addition)
   and your vault (removal).

## Addon-level parity (enforced)

Each addon under `system/addons/<id>/` is a small ksc unit of its own and
`scripts/checks/check-addons.sh` enforces the discipline (doctor-wired, so drift fails
loudly):

- **A `README.md` is mandatory** — every addon documents itself.
- **Manifest ⇄ changelog version parity** — when an addon ships a `CHANGELOG.md`,
  its top versioned heading (`## [x.y.z]`) must equal the manifest `version:`.
  This is the SemVer + Keep-a-Changelog invariant applied per addon; it catches a
  manifest whose version has drifted from its own changelog.
- **Adopted addons version the wrapper, not upstream.** The registry entry's
  `version:` tracks *this* packaging (starting at `0.1.0`), independent of the
  vendored tool's upstream version; record the upstream version in `author:` /
  `upstream:` and the changelog body instead.
...
versioning preferences live in your vault
(`vault/preferences/personal/`), not here.
