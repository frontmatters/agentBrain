...
- **Write** with `new-note.sh --space <slug>`: it roots the note under
  `local/spaces/<slug>/` and stamps a `space: <slug>` field (path-correct UUID5).
- **Seal**: `local/.gitignore` ignores `spaces/`, so a space stays out of the
  personal sync, and default recall (`brain_search` / `brain_recent`) skips it.
...
`relation`, `sync`, `code-roots`) that identifies its owner.
