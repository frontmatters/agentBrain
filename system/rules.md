...
7. `local/sessions/startup-context.md` — live open findings + alerts (skip if absent)
8. Preference scopes — `local/preferences/organization/`, `local/preferences/team/`, `local/preferences/personal/` when present

Treat these compact files as the hot set. Load everything else — `system/tools.md` (bash CLIs + addon binaries), `system/lifecycle.md` (project phases), the relevant `local/projects/[name]/index.md`, `local/memories/`, private preferences detail, integrations, security notes, reports, research, session archives, `system/context-tiers.md` (the tier policy itself), and `system/integrations/opensrc.md` (only when working with a dependency) — as **warm/cold context** when relevant. Measure before adding more always-loaded files.
...
| YouTube transcripts/notes | `local/youtube-digest/` |

### Scripts with config: the mechanism/data mirror

When a shared script needs per-user config or paths, split it across the `system/` ↔ `local/`
...
- **Data** → the mirrored `local/<area>/` (gitignored, user's language). The user copies the
  `.example` here and fills it in.
- Resolve config by precedence — **env override → `local/` config → baked-in default** — using
  `$AGENTBRAIN_DIR`/`$AGENTBRAIN_HOME`, never hardcoded paths.
- **Client-specific** data is a third tier: it belongs in the sealed space
  (`local/spaces/<slug>/index.md`, the paspoort — see `docs/spaces.md`), not general `local/`.
  Mechanisms read the built `local/.space-map.json`, never the passports directly.
...
mirrors `local/references/dev-registry.roots` (private roots), and derives a per-client **Space**
column + canonical/decoy from the space paspoorts via `local/.space-map.json`.
...
compartment at `local/spaces/<slug>/`, kept apart from your personal `local/`.
...
- **Ask when you can't tell but it might be owner-work — never silently misfile.** If
  ...
  the user which owner-space this belongs to (enumerate `local/spaces/*/`) or whether it
...
- **Seal**: `local/.gitignore` ignores `spaces/`, so a space stays out of the personal
...
- **The seal protects SYNC + default RECALL, not access**: an explicit `brain_read`
  of a known `local/spaces/<slug>/…` path still works, and `--space <slug>` on the
  list views opts that one space into a scoped listing.
- **Deliver** a space as a portable package with `brain-extract --space <slug>`;
  `brain-restore` unpacks such a package only back under `local/spaces/<slug>/`.
...
- **Never store secrets in the public layer** (`learnings/`, `system/`, `templates/`, `user-preferences/`, `projects/`) or in the `shared/` layer: no API keys, tokens, passwords, private URLs, customer data, or proprietary code.
