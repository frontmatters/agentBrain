...
| **space** | a sealed per-owner compartment of `local/` (an employer/client) | `local/spaces/<slug>/` (private; gitignored, off default recall) | `new-space.sh <slug>` (create) · `sync-space.sh` (seal) · see `docs/spaces.md` |
...
agentBrain/
  ...
  user-preferences/ youtube-digest/
                       PUBLIC placeholder/template versions of the local/ tree
  local/ -> ~/.agentBrain/vault    PRIVATE layer — WHAT (symlink; gitignored; real knowledge)
...
local/security/  local/youtube-digest/  local/addons/  local/sessions/
```
...
| --- | --- | --- |
| Claude Code | appends an `## agentBrain` pointer block to `~/.claude/CLAUDE.md` (idempotent, marker `# agentBrain`). Skill symlinks (`~/.claude/skills/* → system/skills/*`) are wired by `scripts/setup-skills.sh`; `scripts/check-skill-links.sh` guards against drift. | `setup-claude-code.sh` |
...
`system/pi-config/skills/`. Private or personal skills live under `local/skills/`;
they are discovered and linked by `scripts/setup-skills.sh` and guarded by
`scripts/check-skill-links.sh`. Incoming skills should be reviewed — frontmatter, dedup,
...
- `local/ -> ~/.agentBrain/vault` — the same private vault, symlinked identically into
