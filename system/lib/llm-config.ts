...
export async function resolveUserLlmConfig(
  ...
  if (!root) return null;
  const cfg = await loadJson(join(root, "local", "config", "llm.json"));
...
export async function resolveLlmConfig(
  ...
  const local = opts.skillConfig
    ?? (await loadJson(join(root, "local", "addons", opts.slug, "config.json")));
  const userCfg = await loadJson(join(root, "local", "config", "llm.json"));
