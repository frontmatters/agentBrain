...
Existing public paths use **Title Case** for folders and some files:

```
system/    learnings/    projects/    templates/
sessions/  "daily-notes/"  "user-preferences/"  "youtube-knowledge/"
```

These are **stable API** — do not rename. They are referenced by agents, scripts,
skills, and Obsidian configs. Renaming would break links and tooling.

For **new** public files and folders:

- Prefer **lowercase/kebab-case** unless adding to an existing Title Case folder
- Files: `my-new-note.md` not `My-New-Note.md`
- Folders: `new-folder/` not `New-Folder/`
- Exception: files inside legacy Title Case folders may match the parent convention
