...
vault/learnings/my-learning.md
vault/research/electron-vs-tauri.md
vault/sessions/archive/2026-05/20260518-143205-a7f3.md
```

Legacy mixed-case files in `vault/` exist from before this policy. They are tracked
in `vault/backlog/lowercase-local-path-migration.md` for future cleanup.
...
- Keep archives under `vault/` when they contain real user/project knowledge
...
- **`AGENTBRAIN_DIR`** — brain checkout root. MUST contain `brain.json` + `system/` +
  ...
  (logical `pwd`, NOT `pwd -P`, so symlinked `vault/` is not resolved through).
...
accepted by `check-vault-content` (closes the "false-green by validator-relaxation" risk).
