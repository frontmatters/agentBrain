...
| `new-space` | `S new-space.sh <slug> --owner "<name>" --relation <rel>` | Scaffold a sealed space paspoort (`vault/spaces/<slug>/index.md`) with a fresh `space-id` + path-derived `id`. Then set `sync:` and run `sync-space`. |
...
| `sync-agentbrain-local` | `S sync-agentbrain-local.sh [--dry-run] [message]` | Sync `vault/` and nested spaces to their private remotes. `--dry-run` and `--help` are side-effect free. |
