...
| `new-space` | `S new-space.sh <slug> --owner "<name>" --relation <rel>` | Scaffold a sealed space paspoort (`local/spaces/<slug>/index.md`) with a fresh `space-id` + path-derived `id`. Then set `sync:` and run `sync-space`. |
| `sync-space` | `S sync-space.sh <slug>` | Seal + back up ONE space to its OWN private remote (from the paspoort `sync:`); never the personal vault, never public. `sync: none` = local-only. |
| `build-space-map` | `S build-space-map.sh` | Regenerate `local/.space-map.json` (slug ↔ aliases ↔ code-roots) that context inference reads. |
