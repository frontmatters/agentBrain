...
| `active-space` | `S active-space.sh [use <slug>\|clear\|show]` | **Active space** session mode (incognito-style): `use <slug>` enters a space (validates the slug + that `local/spaces/<slug>/` exists, writes the gitignored `local/.active-space` marker), `clear` exits, `show` prints the active slug. Env `AGENTBRAIN_SPACE` overrides the marker. While active: `new-note` (no `--space`) writes into the space, and MCP recall (`brain_search`/`brain_recent`) surfaces ONLY that space's notes — every other space stays sealed. Resolution order everywhere: env > marker > none. |

> **Spaces — Phase 3 tail (not yet built):** the active-space mode above is the core. Still deferred: `relevant`/still-needed scoping (auto-narrowing recall to the active space's still-relevant notes) and a per-space MOC (map-of-content index). Default recall without an active space is unchanged.
...
|---|---|
| `sync [channel] [--all\|--max=N]` | Iterate configured channels (`local/addons/youtube-digest/channels.json`), fetch latest N videos per channel, dedup against `state.json`. No-arg = all channels (cron-friendly). |
...
Test scripts (`scripts/test-*.sh`) verify individual subsystems — same story: `ls scripts/test-*.sh` for the current list.
