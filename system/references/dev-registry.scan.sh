...
# Maintainer tool: associative arrays need bash 4. macOS ships 3.2 as /bin/bash;
# re-exec on Homebrew's bash when present, otherwise say so instead of dying
# on "declare: -A: invalid option" halfway through.
if [ "${BASH_VERSINFO[0]}" -lt 4 ]; then
	for _b in /opt/homebrew/bin/bash /usr/local/bin/bash; do
		[ -x "$_b" ] && exec "$_b" "$0" "$@"
	done
	echo "dev-registry.scan: needs bash 4 or newer (brew install bash); running on ${BASH_VERSION}" >&2
	exit 2
fi
...
ROOTS_FILE="${ROOTS_FILE:-$AB/vault/references/dev-registry.roots}"
OUT="${OUT_FILE:-$AB/vault/references/dev-registry.md}"
OVERRIDES="${OVERRIDES_FILE:-$AB/vault/references/dev-registry.overrides}"
SPACE_MAP="${SPACE_MAP:-$AB/vault/.space-map.json}"
OWNERS_FILE="${OWNERS_FILE:-$AB/vault/references/dev-registry.owners}"
BRAIN_LOCAL="$AB/vault"
...
[ -x "$AB/scripts/reports/build-space-map.sh" ] && bash "$AB/scripts/reports/build-space-map.sh" >/dev/null 2>&1 || true
...
BODY=$(mktemp)
{
  echo "_Laatst gescand: $TS · $total mappen (eigen $count_eigen · extern $count_extern · klant $count_klant · lokaal/onbekend $count_lokaal · spaces $count_spaced · decoys $count_decoy). Roots: ${ROOTS_STR}. Herkomst uit git-remote; Space/canoniek uit vault/.space-map.json (paspoorten); \`lokaal?\` = geen remote._"
  echo
