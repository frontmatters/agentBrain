...
`local/skills/<name>/SKILL.md` — one source, many connectors, no manual
...
Unlike `system/skills/` and `local/skills/`, an addon's skill is **only**
linked while the addon is enabled (`local/addons/<id>/enabled` present) — the
link is created/removed automatically by:

- `bash scripts/addons.sh enable|disable <id>` — syncs just that addon's link
  for every agent (`sync_addons_for_agent` in `scripts/setup-skills.sh`).
