...
remove. The only state is runtime events in `vault/events/`:

```bash
bash system/addons/event-bus/uninstall.sh --purge   # delete vault/events/ state
...
$AGENTBRAIN_DIR/system/addons/event-bus/bin/brain-emit \
    ...
    --to=pi \
    --from=claude \
    --payload='{"document":"vault/skills/event-bus/SPEC.md","question":"is this design sound?"}'
```
...
The spec proposes propagating events via gitea-sync of `vault/events/`. For v1
the addon runs **local-only** — `vault/events/` is gitignored as runtime state.
