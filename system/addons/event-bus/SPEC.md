...
> `IMPL` — `scripts/checks/check-events.sh` runs in doctor and validates every event in
> `vault/events/inbox/` + `vault/events/archive/` against this schema (required
...
```bash
find $AGENTBRAIN_DIR/vault/events/inbox -name "*.json" \
...
- v1-v4 (monolithic): `vault/backlog/2026-05-24-internal-event-bus-design.md` — 30KB doc with internal drift across 4 Pi review rounds
- v5 (decomposition): 9 separate topic-specs in `vault/skills/event-bus/`
