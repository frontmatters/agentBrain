...
compaction**, writing them into `vault/learnings/extracted/`. It runs as a Claude
...
Each learning becomes one file in `vault/learnings/extracted/`, named
...
(`vault/addons/youtube-digest/channels.json` → `settings`) when present, else it
...
the real `vault/learnings/extracted/` or the network.

## Backend & privacy

The extractor sends the (flattened) session transcript to an LLM. Which one:

1. the summarizer configured in `vault/addons/youtube-digest/channels.json`
