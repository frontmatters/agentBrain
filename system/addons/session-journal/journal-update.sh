...
# Core: read a Claude Code transcript JSONL and update vault/sessions/session-journal.md
# Usage:
...
#
# Reads config from vault/sessions/journal-config.json (falls back to addon default).
...
JOURNAL="$BRAIN_ROOT/vault/sessions/session-journal.md"
LOCAL_CONFIG="$BRAIN_ROOT/vault/sessions/journal-config.json"
...
try:
    c = json.load(open('$config_path'))
    print(c.get('general',{}).get('log_path','vault/sessions/.journal-hook.log'))
except Exception:
    print('vault/sessions/.journal-hook.log')
" 2>/dev/null || echo 'vault/sessions/.journal-hook.log')"
	log_rel="vault/${log_rel#local/}"; log_rel="${log_rel#vault/vault/}"   # an older config still says local/
...
# --- Route the journal to the session's context (S5: context-aware routing) ---
# The journal must not copy a client's identifiers out of the chmod-700 _work/
# boundary into the shared vault. Resolve the session's cwd to a space and route
# there; a _work/ session with no mapped space fails closed to a gitignored
# local-only quarantine, never the shared personal vault.
session_cwd=""
...
journal_root="$BRAIN_ROOT/vault/sessions"
if [[ -n "$session_cwd" ]]; then
	ctx="unknown"
	if [[ -r "$BRAIN_ROOT/system/lib/context.sh" ]]; then
		# shellcheck source=/dev/null
		ctx="$( cd "$BRAIN_ROOT" && . system/lib/context.sh && infer_context "" "$session_cwd" 2>/dev/null || echo unknown )"
	fi
	case "$ctx" in
		""|unknown)
			if [[ "$session_cwd" == *"/_work/"* ]]; then
				journal_root="$BRAIN_ROOT/vault/sessions/.quarantine"
				log "confidential cwd, no space ($session_cwd) -> quarantine"
			fi
			;;
		ambiguous)
			log "ambiguous context for $session_cwd -> skipping journal (refuse, do not guess)"
			exit 0
			;;
		*)
			journal_root="$BRAIN_ROOT/vault/spaces/$ctx/sessions"
			log "routed journal to space '$ctx'"
			;;
	esac
fi
JOURNAL="$journal_root/session-journal.md"
