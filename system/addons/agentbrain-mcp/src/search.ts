...
import { loadAccessIndex, accessWeight } from "./access";
...
export function skipPrefixes(): string[] {
  ...
  let lines: string[];
  try {
    lines = readFileSync(join(localDisk(), ".searchignore"), "utf8").split("\n");
  } catch {
...
// rather than throwing. Keys and synonyms are lowercased so lookup matches the
// already-lowercased query terms. Loaded once per search(), never per file.
export function searchMap(): Map<string, string[]> {
  const map = new Map<string, string[]>();
  let lines: string[];
  try {
    lines = readFileSync(join(localDisk(), ".searchmap"), "utf8").split("\n");
  } catch {
    return map; // absent file is the normal case
  }
  for (const raw of lines) {
    const line = raw.trim();
    if (!line || line.startsWith("#")) continue;
    const idx = line.indexOf(":");
    if (idx < 1) continue; // no colon, or empty key
...
export interface Hit {
  ...
  score: number;
  /**
   * Substitutions this hit needed to match, as "queryTerm→matched": a typo or
   * inflection ("tokne→token") or a `.searchmap` synonym ("verwijderen→opschonen").
   * Absent on exact hits.
   */
  fuzzy?: string[];
}
...
export async function search(query: string, limit = 20): Promise<Hit[]> {
  ...
  interface Entry extends Hit {
    ...
    exact: number;
    fuzz: number;
    /** Recency-decayed access count; a tie-breaker only, filled just before the sort. */
    weight: number;
  ...
  // Expansion candidates per term. Two sources feed the SAME crediting pass:
  //   - fuzzy substitutes (typos/inflections), only for terms that almost missed;
  //   - static-map synonyms (Layer A), for every term that is a map key.
  // Both are scored in the lower FUZZY_WEIGHT tier and reported in `fuzzy`.
  ...
  // Layer A — synonym expansion. Runs for every query term that is a map key,
  // independent of the weak/fuzzy trigger: a synonym is a DIFFERENT word, so a note
  // carrying only it was invisible to pass 1 no matter how common the term is. Map
  // synonyms are listed BEFORE any fuzzy candidates so that when several substitutes
  // match one note, the synonym — the stronger signal — is the one reported.
  const smap = searchMap();
  for (const t of terms) {
    const syns = smap.get(t);
    if (!syns) continue;
    expansions.set(t, [...syns, ...(expansions.get(t) ?? [])]);
  }

  // Pass 3 — credit notes that carry a substitute (synonym or typo) for a term the
  // query did not match verbatim.
  ...
  // Usage is a tie-breaker, not a score component: it is read once here and never blended
  // into exact/fuzz. Notes with no access record weigh 0 and sort last within their tier.
  const accessIdx = loadAccessIndex();
  for (const e of scored) e.weight = accessWeight(accessIdx[e.path]);
  ...
  // guesses can ever displace a note that actually contains what was asked for. Access
  // weight ranks below all of those — it only orders notes that are otherwise equal.
  scored.sort(
    (a, b) =>
