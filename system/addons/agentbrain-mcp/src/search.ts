...
// "Active space" session mode (Phase 3): when a space is active, recall operates
// INSIDE it — its notes ARE surfaced while every OTHER space stays sealed. Resolved
// exactly like the bash side (scripts/active-space.sh): env AGENTBRAIN_SPACE wins,
// else the local/.active-space marker under the same brain root, else none. Returns
// "" when no space is active (= default behavior: all spaces excluded).
export function activeSpace(): string {
  const env = process.env.AGENTBRAIN_SPACE;
  if (env && env.trim()) return env.trim();
  try {
    return readFileSync(join(brainRoot(), "local", ".active-space"), "utf8").trim();
  } catch {
    return "";
  }
}
