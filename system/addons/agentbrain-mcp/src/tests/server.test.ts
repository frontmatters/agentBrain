...
test("brain_search description tells the caller to enrich with synonyms", async () => {
  const src = readFileSync(new URL("../server.ts", import.meta.url), "utf8");
  assert.match(
    src,
    /synonym|domain term/i,
    "the brain_search tool description must advertise query enrichment (layer C)",
  );
});

test("brain_read records an access bump in the sidecar", { timeout: 15000 }, async () => {
  await talk([
    { jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "test", version: "0" } } },
    { jsonrpc: "2.0", method: "notifications/initialized" },
    { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "brain_read", arguments: { path: "local/learnings/x.md" } } },
  ]);
  const idx = JSON.parse(readFileSync(join(root, "local", ".access-index.json"), "utf8"));
  assert.ok(idx["local/learnings/x.md"], "brain_read bumped the access index");
  assert.equal(idx["local/learnings/x.md"].count, 1);
});

test("brain_read via the server records the canonical key search will look up", { timeout: 15000 }, async () => {
  await talk([
    { jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "test", version: "0" } } },
    { jsonrpc: "2.0", method: "notifications/initialized" },
    { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "brain_read", arguments: { path: "./local/learnings/x.md" } } },
  ...
  const idx = JSON.parse(readFileSync(join(root, "local", ".access-index.json"), "utf8"));
  assert.ok(idx["local/learnings/x.md"], "canonical key present for a non-canonical read path");
});
