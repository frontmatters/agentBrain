...
test("searchMap parses key: synonyms, lowercased and trimmed", async () => {
  process.env.AGENTBRAIN_DIR = root;
  writeFileSync(
    join(root, "local", ".searchmap"),
    "# a comment\n\nVerwijderen: Opschonen, wissen ,  forget\nmcp: server, tool\n",
  );
  const { searchMap } = await import("../search");
  const m = searchMap();
  assert.deepEqual(m.get("verwijderen"), ["opschonen", "wissen", "forget"]);
  assert.deepEqual(m.get("mcp"), ["server", "tool"]);
  rmSync(join(root, "local", ".searchmap"), { force: true });
});

test("searchMap skips malformed lines without throwing", async () => {
  process.env.AGENTBRAIN_DIR = root;
  writeFileSync(
    join(root, "local", ".searchmap"),
    "no-colon-here\n: no key\nkey-no-syns:   \nok: yes\n",
  ...
  const { searchMap } = await import("../search");
  const m = searchMap();
  assert.equal(m.has("no-colon-here"), false, "no colon → skipped");
  assert.equal(m.has(""), false, "empty key → skipped");
  assert.equal(m.has("key-no-syns"), false, "no synonyms → skipped");
  assert.deepEqual(m.get("ok"), ["yes"]);
  rmSync(join(root, "local", ".searchmap"), { force: true });
});

test("searchMap is empty when the file is absent", async () => {
  process.env.AGENTBRAIN_DIR = root;
  rmSync(join(root, "local", ".searchmap"), { force: true }); // guarantee the precondition
  const { searchMap } = await import("../search");
  assert.equal(searchMap().size, 0);
});

// Layer A: a synonym in the map reaches a note that never spells the query term.
// "opschonen" is a different WORD, not a typo — pass 1 and fuzzy both miss it; only
// the map bridges it. Scored in the fuzzy tier, reported as term→synonym.
writeFileSync(
  join(root, "local", "learnings", "cleanup.md"),
  "---\ntitle: Cleanup\n---\n\nJe kunt oude notes opschonen wanneer nodig.\n",
);
writeFileSync(
  join(root, "local", "learnings", "removal.md"),
  "---\ntitle: Removal\n---\n\nJe kunt een note verwijderen met de forget-skill.\n",
);

test("a mapped synonym reaches a note that lacks the query term, in the fuzzy tier", async () => {
  process.env.AGENTBRAIN_DIR = root;
  writeFileSync(join(root, "local", ".searchmap"), "verwijderen: opschonen\n");
  const { search } = await import("../search");
  const hits = await search("verwijderen");
  const syn = hits.find((h) => h.path === "local/learnings/cleanup.md");
  const exact = hits.find((h) => h.path === "local/learnings/removal.md");
  assert.ok(syn, "the synonym-only note is found");
  assert.deepEqual(syn.fuzzy, ["verwijderen→opschonen"], "provenance reported");
  assert.ok(syn.score < 1, `synonym scores in the lower tier, got ${syn.score}`);
  assert.ok(exact, "the verbatim note is found");
  assert.ok(
    hits.indexOf(exact) < hits.indexOf(syn),
    "a verbatim hit outranks a synonym hit",
  );
  rmSync(join(root, "local", ".searchmap"), { force: true });
});

test("a term matched verbatim is not also credited via its synonym", async () => {
  process.env.AGENTBRAIN_DIR = root;
  // removal.md contains "verwijderen" verbatim; the map also maps it to "forget",
  // which removal.md ALSO contains. It must be paid once, not twice.
  writeFileSync(join(root, "local", ".searchmap"), "verwijderen: forget\n");
  const { search } = await import("../search");
  const hits = await search("verwijderen");
  const both = hits.find((h) => h.path === "local/learnings/removal.md");
  assert.ok(both);
  assert.equal(both.score, 1, "verbatim match already credited the term — the synonym must not pay it again");
  assert.equal(both.fuzzy, undefined);
  rmSync(join(root, "local", ".searchmap"), { force: true });
});

test("synonym expansion runs even when the query term is common (not weak)", async () => {
  process.env.AGENTBRAIN_DIR = root;
  // Make "verwijderen" appear in enough notes that it is NOT a weak (fuzzy-eligible)
  // term, then confirm the synonym-only note is still reached — proving synonym
  // expansion does not piggy-back on the fuzzy weak-term trigger.
  for (let i = 0; i < 4; i++) {
    writeFileSync(
      join(root, "local", "learnings", `common-${i}.md`),
      `---\ntitle: Common ${i}\n---\n\nHier kun je iets verwijderen.\n`,
    );
  }
  writeFileSync(join(root, "local", ".searchmap"), "verwijderen: opschonen\n");
  try {
    ...
    const hits = await search("verwijderen");
    const syn = hits.find((h) => h.path === "local/learnings/cleanup.md");
    assert.ok(syn, "synonym-only note reached despite a common query term");
    assert.deepEqual(syn.fuzzy, ["verwijderen→opschonen"]);
  } finally {
    // Clean up even if an assertion throws — else these four notes leak into the
    // shared temp vault and skew filesPerTerm ("verwijderen") for any later test.
    rmSync(join(root, "local", ".searchmap"), { force: true });
    for (let i = 0; i < 4; i++) rmSync(join(root, "local", "learnings", `common-${i}.md`), { force: true });
  }
});

// Cross-feature: a multi-term query where ONE term matches verbatim and ANOTHER only
// via a .searchmap synonym, both on the SAME note. The note must collect both credits
// (exact tier + fuzzy tier) and report only the synonym in `fuzzy`. This interaction
// between the exact pass and Layer A is invisible to any single-term test.
writeFileSync(
  join(root, "local", "learnings", "mixed.md"),
  "---\ntitle: Mixed\n---\n\nDe migratie kreeg een rollback.\n",
);

test("a verbatim term and a mapped synonym combine on one note", async () => {
  process.env.AGENTBRAIN_DIR = root;
  writeFileSync(join(root, "local", ".searchmap"), "terugdraaien: rollback\n");
  try {
    const { search } = await import("../search");
    const hits = await search("migratie terugdraaien");
    const mixed = hits.find((h) => h.path === "local/learnings/mixed.md");
    assert.ok(mixed, "found via one exact term ('migratie') + one synonym ('terugdraaien'→'rollback')");
    assert.equal(mixed.score, 1.5, "1 for the exact term + 0.5 for the synonym");
    assert.deepEqual(mixed.fuzzy, ["terugdraaien→rollback"], "only the synonym term is reported");
  } finally {
    rmSync(join(root, "local", ".searchmap"), { force: true });
  }
});

// Two notes tie on exact=1 for "tiebreakword". Named so alpha.md sorts BEFORE omega.md
// on path — so if omega wins, only the access weight could have done it.
writeFileSync(
  join(root, "local", "learnings", "alpha.md"),
  "---\ntitle: Alpha\n---\n\nA unique tiebreakword here.\n",
);
writeFileSync(
  join(root, "local", "learnings", "omega.md"),
  "---\ntitle: Omega\n---\n\nA unique tiebreakword here.\n",
);

test("access weight breaks a tie within a score tier (and cleans up after itself)", async () => {
  process.env.AGENTBRAIN_DIR = root;
  const { recordAccess } = await import("../access");
  recordAccess("local/learnings/omega.md");
  const { search } = await import("../search");
  const hits = await search("tiebreakword");
  assert.equal(hits.length, 2, "both notes tie on the exact term");
  assert.equal(hits[0].path, "local/learnings/omega.md", "the accessed note wins the tie…");
  assert.equal(hits[1].path, "local/learnings/alpha.md", "…over the alphabetically-earlier one");
  rmSync(join(root, "local", ".access-index.json"), { force: true }); // keep other tests index-free
});
