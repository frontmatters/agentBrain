...
const checkLocalContent = {
  ...
  findings: [
    {
      id: "check-vault-content:dead-link:local/foo.md:bar",
      severity: "warning",
      file: "local/foo.md",
      kind: "dead-link",
    ...
    {
      id: "check-vault-content:missing-id:local/baz.md",
      severity: "error",
      file: "local/baz.md",
      kind: "missing-id",
      message: "missing id",
    ...
    {
      id: "check-vault-content:dead-link:local/old.md:gone",
      severity: "warning",
      file: "local/old.md",
      kind: "dead-link",
...
writeFileSync(
  join(root, "local", "findings", "check-vault-content.json"),
...
test("listFindings returns all findings across detectors when unfiltered", async () => {
  ...
  assert.ok(detectors.includes("check-vault-content"));
