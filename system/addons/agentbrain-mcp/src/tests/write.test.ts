...
test("incognito flag is honoured when the private layer is vault/ (a current install has no local/)", async () => {
  process.env.AGENTBRAIN_DIR = root;
  const { saveLearning } = await import("../write");
  // Before 0.2.1 the flag was checked at <root>/local/sessions/.incognito, a path
  // a checkout with a vault/ link never has, so incognito silently stopped blocking.
  mkdirSync(join(root, "vault", "sessions"), { recursive: true });
  const flag = join(root, "vault", "sessions", ".incognito");
  writeFileSync(flag, "incognito enabled\n");
  try {
    await assert.rejects(() => saveLearning("Vault Incognito", "should not persist", []), /incognito/i);
  } finally {
    rmSync(flag);
  }
});
