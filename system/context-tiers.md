...
The canonical hot reading list is **`system/rules.md` Step 1** (identical to the installed pointer block) — this file does not restate it. In short: the compact public files above, plus `vault/sessions/startup-context.md` and preference scopes when present. Hot context should be compact, stable, and broadly useful.

## Warm context

Load on demand when relevant:

- `vault/preferences/{organization,team,personal}/`
- `vault/projects/<name>/index.md` and optional project files
- `vault/learnings/`
- `vault/integrations/` and `vault/security/` before credential-sensitive work
- skill reference files and templates

Warm context may contain real user/project knowledge and therefore belongs in `vault/` when private.
...
- Public files describe HOW/WHERE only; real WHAT stays in `vault/`.
