...
- VS Code Copilot: `settings.json` written by `scripts/setup/setup-copilot.sh`
- Cursor: `~/.cursor/User/settings.json` written by `scripts/setup/setup-cursor.sh`

All of the above are run during setup by `scripts/setup/setup-agent-integrations.sh` (which `scripts/setup/setup.sh` calls), each only when its client is detected.
