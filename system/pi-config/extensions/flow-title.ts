...
function sampleGradient(position: number) {
	...
	const a = PALETTE[index] ?? DEEP_BLUE;
	const b = PALETTE[nextIndex] ?? DEEP_BLUE;
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", (_event, ctx) => {
		// A fresh machine without a provider reports id "unknown" — show the fix,
		// not a cryptic label.
		const _id = ctx.model?.id;
		currentModelId = !_id || _id === "unknown" ? "no model — /login" : _id;
		if (!ctx.hasUI) return;
		installHeader(ctx);
	});

	pi.on("model_select", (event) => {
		currentModelId = event.model.id === "unknown" ? "no model — /login" : event.model.id;
		requestRender?.();
	});
