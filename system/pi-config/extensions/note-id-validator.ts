 ...
 *
 * The commit-time gate (scripts/hooks/validate-staged-note-ids.sh) is the true
 * agent-agnostic backstop; these handlers are fast per-write feedback.
 *
 * Shared validator: scripts/hooks/validate-note-id.sh (called via spawnSync). It owns
...
import { brainDir, extractFilePath } from "./brain-paths";
...
const VALIDATOR = join(brainDir(), "scripts", "validate-note-id.sh");
