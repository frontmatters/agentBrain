...
function registerAgentBrainProjectContextTool(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		description:
			"Load the detected or named project context from agentBrain/vault/projects/<project> including index, PRD, decisions, deploy, changelog, and context files when present.",
		promptSnippet:
			"Load project-specific persistent memory from agentBrain/vault/projects",
		promptGuidelines: [
		...
		parameters: Type.Object({
			project: Type.Optional(
				Type.String({
					description:
						"Project folder name under vault/projects; omit for auto-detection",
				}),
			),
		}),
