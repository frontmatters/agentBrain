 ...
 * `vault/` tree. Fires on `tool_call` (before the tool runs) so the write is
 * prevented, not merely flagged after the fact.
 *
 * Scope: only `vault/` knowledge is blocked. Code edits (system/, scripts/,
...
const BLOCK_REASON =
	...
	"Writing knowledge to vault/ is suppressed. Consulting the brain still " +
