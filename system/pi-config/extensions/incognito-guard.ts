...
import { isIncognito, isVaultLocalWrite } from "./brain-paths";
...
export default function incognitoGuard(pi: ExtensionAPI): void {
	pi.on("tool_call", (event: any) => {
		const toolName: string = event?.toolName ?? event?.input?.tool ?? "";
		...
		if (!isVaultLocalWrite(filePath)) return undefined;
