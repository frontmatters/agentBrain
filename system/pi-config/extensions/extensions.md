...
| `note-id-validator.ts`  | Validates brain-note `id:` frontmatter after Write/Edit (shells out to `scripts/validate-note-id.sh`) | `tool_result`                                                             |
...
| `pi.on("tool_call", ...)`                 | `git-interceptor.ts`, `incognito-guard.ts`                   | `ExtensionAPI.on`                   |
