 ...
 * vault.  Import `brainPath` instead of calling `path.join(brainDir(), ...)`
...
// Resolved on every call, not at import: in one `bun test` process several
// test files each set AGENTBRAIN_DIR, and a load-time constant would hold
// whichever value the first import saw (the same lesson as the MCP's brainRoot()).
export function brainDir(): string {
	return path.resolve(resolveBrainDir());
}
...
/**
 * True when `filePath` is a write into the active vault's `vault/` knowledge tree.
