...
//   1. The shared validator (scripts/hooks/validate-note-id.sh) exists and works
...
test("scripts/hooks/validate-note-id.sh exists and is executable", () => {
...
test("validator returns 0 on valid id, 1 on mismatch (smoke from extension perspective)", () => {
	...
	// brain.json is untracked (setup-brain-config.sh generates it) — a fresh
	// checkout/worktree has none. The test only needs A namespace: uuid5-gen
	// and the validator both read the same fixture copy, so a stub is exact.
	const brainJson = join(REPO_ROOT, "brain.json");
	if (existsSync(brainJson)) {
		copyFileSync(brainJson, join(fixture, "brain.json"));
	} else {
		writeFileSync(
			join(fixture, "brain.json"),
			JSON.stringify({ namespace: "6ba7b810-9dad-11d1-80b4-00c04fd430c8" }),
		);
	}
