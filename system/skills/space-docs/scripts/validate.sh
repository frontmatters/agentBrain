...
MDS=(); while IFS= read -r -d '' _md; do MDS+=("$_md"); done < <(find "$dir" -type f -name '*.md' -not -path '*/.git/*' -print0)   # a read loop: bash 3.2 (macOS /bin/bash) has no array-from-lines builtin
