...
```
~/agentBrain/
├── vault/        ← user content (learnings, projects, sessions, references, backlog, addons)
...
bash ~/agentBrain/scripts/checks/doctor.sh
```
...
grep -roh "\[\[[^\]]*\]\]" ~/agentBrain/local/ \
    | sed -E 's/\[\[([^|\]]+).*\]\]/\1/' | sort -u \
    | while read t; do
        # crude resolution: find <slug>.md anywhere under vault/
...
- **Recurring problems** → should move to `vault/learnings/troubleshooting.md`
- **Recurring decisions** → should move to the agent-guidance file (`CLAUDE.md` / `AGENTS.md` / `GEMINI.md`, per agent)
- **Recurring workarounds** → should move to `vault/learnings/patterns.md`
- **New projects** that don't have a `vault/projects/<name>/` note yet
...
```bash
# Open backlog items
grep -lE "^status:[[:space:]]*open" ~/agentBrain/vault/backlog/*.md \
...
- Suggest patterns from daily-notes that should be promoted to permanent notes (`vault/references/` or `vault/learnings/`)

The `weekly-review` addon (`vault/sessions/weekly/<YYYY-WNN>.md`) already records `source_daily_notes` in its frontmatter — count = 0 is a signal that the daily-note habit isn't sticking yet.

## Output

After the review, write a report at `~/agentBrain/vault/sessions/<YYYY-MM-DD>-vault-review.md`:
...
projects: [agentbrain]
status: completed
id: <UUID5 via scripts/uuid5-gen.sh "vault/sessions/<YYYY-MM-DD>-vault-review">
---
