...
- **READ `system/explainers/copy-norms.md` BEFORE drafting any visible copy, and
  apply every rule while writing** — not as a fix-up pass. That file is the single
  source of truth for output copy (active voice, max 25-word sentences, one term
  per concept, concrete over qualitative, standard punctuation). `check-explainers.sh`
  enforces it mechanically. The most common re-render cause is rule 6: the gate
  auto-fails em-dashes (`—`), en-dashes (`–`) and double-hyphens (`--`); write
  em-dash-free from the first draft (use a period, colon, or parentheses instead).
