...
- **Deprecate user-facing renames, don't hard-break** — when renaming an addon or a
  user-invocable skill (anything a user types by name), leave the old name as a
  `deprecated:` alias instead of deleting it outright. Add the block to the alias's
  `manifest.md` / `SKILL.md` frontmatter — old invocations keep working during a
  window, `addons.sh status` / `skills list` surface a pointer, and
  `check-deprecations.sh` nags once `remove_after` passes:
  ```yaml
  deprecated:
    since: <today>
    reason: renamed          # renamed | replaced | merged | discontinued
    replaced_by: <new-name>  # omit only when reason=discontinued
    remove_after: <today+90d>
  ```
  Full contract: `system/addons/README.md` (### Deprecation). File the removal as a
  dated backlog note for `remove_after`.
...
sessions/
daily-notes/
templates/vault/preferences/personal/
youtube-digest/
```
...
vault/preferences/git-config.md
vault/integrations/azure-devops.md
vault/security/gitea.md
```
...
- `/refactor-brain rename vault/preferences/Tech Stack.md to vault/preferences/tech-stack.md`
- `/refactor-brain move old integration notes into vault/integrations/`
...
- Separate public paths from `vault/` paths.
...
- Do not overwrite `vault/` content.
...
```text
Rename vault/preferences/Tech Stack.md -> vault/preferences/tech-stack.md
```
...
```text
FILES TO RENAME
  vault/preferences/Tech Stack.md -> vault/preferences/tech-stack.md

REFERENCES TO UPDATE
  vault/preferences/README.md:12 Tech Stack.md -> tech-stack.md
