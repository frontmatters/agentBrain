...
- Session sources:
  - `~/.claude/projects/*/*.jsonl` for Claude Code transcripts (via `scripts/flow/session-digest.sh`).
  - `~/.pi/agent/sessions/` for Pi JSONL sessions (same digest script).
  - `vault/sessions/` for agentBrain session journals and archives.
- Knowledge sources:
...
2. **Collect session evidence (digest-first)**
   - Run the deterministic digest over both session sources:
     `bash scripts/flow/session-digest.sh --days <N> --out vault/sessions/digests/$(date +%F)-digest.md`
   ...
   - Read `vault/sessions/session-journal.md` and recent `vault/sessions/archive/YYYY-MM/*.md` when present.

3. **Collect brain context**
   - Read active project indexes from `vault/projects/*/index.md`.
   - Read relevant `vault/learnings/` notes.
...
5. **Write a private report**
   - Save to `vault/reports/brain-insights-YYYYMMDD.md` unless the user asks for chat-only output.
   - Include frontmatter:
     ```yaml
     ---
     date: YYYY-MM-DD
     ...
     ---
     ```
   - Generate UUID5 with `scripts/uuid5-gen.sh "vault/reports/brain-insights-YYYYMMDD"`.
...
- Reports are private and must stay under `vault/`.
