...
Read-only view that enumerates notes carrying `hidden: true`, plus — with `--include-trash` — forgotten batches in `vault/.trash/forget/`. Each row is numbered so `/brain-unhide <N>` and `/brain-recall <N>` can resolve by index.
