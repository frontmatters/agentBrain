...
| ---------------------------------- | -------------------------------------------------------------------------------------- |
| Physical machine/host/device       | `vault/devices/[name].md` (see device frontmatter note below)                          |
| Integration endpoint/helper/config | `vault/integrations/[tool].md`                                                         |
| Credential storage/auth workflow   | `vault/security/[tool].md` or relevant integration note                                |
| Git provider preferences           | `vault/preferences/personal/git-config.md`                                             |
| Azure/cloud preferences            | `vault/preferences/personal/cloud-azure.md`                                            |
| Organization/work context          | `vault/preferences/organization/context.md` or `vault/preferences/team/context.md`     |
| Organization/internal libraries    | `vault/preferences/organization/libraries.md` or `vault/preferences/team/libraries.md` |
| Project-specific tool info         | `vault/projects/[project]/`                                                            |
| Troubleshooting fix                | `vault/learnings/troubleshooting.md`                                                   |
| Reusable pattern                   | `vault/learnings/patterns.md`                                                          |
| Setup event/history                | `vault/setup-history/[topic].md`                                                       |
| Future idea                        | `vault/backlog/[topic].md`                                                             |
...
```bash
scripts/uuid5-gen.sh "vault/integrations/tool-name"
```

### Device notes are different (`vault/devices/`)

A physical machine/host is a **device**, not an integration — `vault/devices/`
...
### CMDB relations (optional, recommended)

When capturing a device or integration, also record its typed CMDB relations as
frontmatter YAML lists so `brain-cmdb` can traverse them:

- `hosted_on: [<host-slug>]` — the machine/host this runs on.
- `depends_on: [<ci-slug>]` — CIs it needs to function.
- `provides: [<ci-slug>]` — CIs it serves.

Values are other CI note slugs (a device or integration filename without `.md`).
After editing relations, run `brain-cmdb sync` to regenerate the note's `## Relations`
section. See the `brain-cmdb` skill.
...
- `vault/preferences/personal/git-config.md`
- `vault/security/credential-notes.md`
```
...
- Approved corporate/work details still go only to `vault/` or configured `vault/secure/`, never public docs.
