---
name: brain-purge
description: PERMANENTLY delete forget-batches from agentBrain's trash (vault/.trash/forget/). Always previews + validates first; deletion requires explicit user confirmation — never skippable. Counterpart that makes /brain-forget final. Triggers — "purge", "echt verwijderen", "definitief weg", "leeg de trash", "empty trash".
---

# brain-purge — permanent delete from trash

Makes a [[brain-forget]] final: removes a trash batch from `vault/.trash/forget/<timestamp>/` **permanently**. After purge, `/brain-recall` can no longer restore it.
