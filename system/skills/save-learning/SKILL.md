...
Public = HOW/WHERE. Private = WHAT. Save real discoveries in `vault/`.
...
under `vault/spaces/<slug>/learnings/` with a `space:` field and path-correct UUID5,
...
```bash
bash scripts/new-note.sh learning vault/learnings/<name> --context <slug>
```
...
NOT save to personal — ask the user which owner-space (enumerate `vault/spaces/*/`) or
whether it is personal, then pass `--context <slug>`. See the Spaces / ownership policy
in `system/rules.md`.

**Never** write a space's learning to the shared `vault/learnings/` — that leaks
...
1. **Determine the type of insight:**
   - Recurring pattern (seen 2x+) -> add to `vault/learnings/patterns.md`
   - Pattern seen 1st time -> add to `vault/learnings/patterns.md` with `confidence: low`, note "seen 1x"
   - New technical insight -> create `vault/learnings/[category].md` (lowercase/kebab-case)
   ...
   **Learnings are flat files + frontmatter `tags` (+ UUID + `[[wiki-links]]`), not folders.**
   ...
   (machine-generated auto-extraction). Enforced by `scripts/checks/check-learnings-structure.sh`.
...
- Existing patterns: `vault/learnings/patterns.md`
