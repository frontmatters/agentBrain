...
**Skill location**: `$BRAIN_DIR/vault/skills/promote/` (where `$BRAIN_DIR` is the realpath of `~/agentBrain`)
...
3. **Compute target via prefix-strip-and-prepend**:
   ...
   - This avoids substring-replace edge cases when a path contains `vault/` twice (e.g., `$BRAIN_DIR/vault/skills/local/foo.md`).
...
Identical to `promote` with `vault/` and `system/` swapped in step 2 and step 3 (strip `$BRAIN_DIR/system/`, prepend `$BRAIN_DIR/local/`).
...
| Target exists | Refuse with exit 2. `--force` first moves the existing target to **central trash**: `$BRAIN_DIR/vault/.trash/promote/<YYYYMMDD-HHMMSS>/<relative-target-path>`, preserving structure. Never `rm`. The `.trash/` folder is gitignored (see section 8). |
...
| User passes a relative path like `vault/skills/foo` | Normalize via `realpath` (with fallback); result must start with `$BRAIN_DIR/local/`. |
...
| Cross-filesystem `mv` | Theoretically possible (e.g., if user puts `vault/` and `system/` on different mounts), but `~/agentBrain` lives on one disk in this user's setup. Out of scope to guard for v1 — see section 8. |

## 7. File layout

```
~/agentBrain/vault/skills/promote/
...
- **Trash folder**: `$BRAIN_DIR/vault/.trash/promote/` — append `vault/.gitignore` line `/.trash/` (or `.trash/` if at root) during skill setup. Never auto-`rm`.
...
- ~~`--force` semantics~~ → **central trash folder pattern adopted** (`$BRAIN_DIR/vault/.trash/promote/<ts>/…`), gitignored. See section 6 + 8.
