...
- `--space <slug>` — opt-in view of a single space's own projects
  (`local/spaces/<slug>/projects`) instead of the default view. Space
...
The default view globs `local/projects/*/index.md` only, so space content
(`local/spaces/<slug>/projects/`) never appears unless `--space <slug>` is
