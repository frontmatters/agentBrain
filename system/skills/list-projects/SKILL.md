...
`vault/projects/`. Sibling of [[list-parks]] but unfiltered by status; the
broader catalog.
...
- `--space <slug>` — opt-in view of a single space's own projects
  (`vault/spaces/<slug>/projects`) instead of the default view. Space
...
- `Read ~/agentBrain/vault/projects/<name>/index.md` — inspect
...
bash ~/agentBrain/system/skills/list-projects/bin/list-projects --space <slug>    # opt-in: a space's own projects
bash ~/agentBrain/system/skills/list-projects/bin/list-projects --json
```

The default view globs `vault/projects/*/index.md` only, so space content
(`vault/spaces/<slug>/projects/`) never appears unless `--space <slug>` is
