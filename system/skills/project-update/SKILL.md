...
> infers the space per-write (code-root / `AGENTBRAIN_CONTEXT`); if the work is
> owner-specific but path signals can't tell, ASK which owner-space (enumerate
> `local/spaces/*/`) or personal, then pass `--context <slug>`. Never silently save
> owner-work to personal. See the Spaces / ownership policy in `system/rules.md`.
