...
- A space lives at `local/spaces/<slug>/` and is **sealed out of the personal
  vault** by `local/.gitignore` (`spaces/`) — it never reaches the personal
...
- Create + seal (full workflow in `docs/spaces.md`):
  ...
  bash ~/agentBrain/scripts/sync-space.sh <slug>
  ```
...
Skip-if-done: if `local/devices/index.md` exists, only ask whether anything
...
- **Devices** → `local/devices/<hostname>.md` from
  `system/templates/device-template.md`, plus one line in the
  `local/devices/index.md` hop-page (create it on first use: a table with one
  row per device — hostname, role, OS, status). Hostnames are canonical for
  wikilinks.
- **Services** → `local/integrations/<service>.md` from
  `system/templates/integration-template.md`.
- Create notes via `bash ~/agentBrain/scripts/new-note.sh` (correct
  frontmatter + UUID5) or render the template and fill it in.
- **Privacy rule, non-negotiable**: reference credentials by helper, env-var,
  or keychain-item NAME — never by value, not even here in `local/`.
