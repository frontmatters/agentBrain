...
1. **Personal preferences** (always) — language, stack, workflow, design and decision preferences under `vault/preferences/personal/`
2. **Organization scope** (optional) — broader organization context under `vault/preferences/organization/`
3. **Team scope** (optional) — team-specific agreements under `vault/preferences/team/`
...
6. **Release channel & updates**: channel (`stable`/`prerelease`/`edge`) via `scripts/channel.sh` and the `auto_update` mode in `vault/update/config.json`
