...
- **new: opt-in trademark clearance** (`sweep-trademark.sh <name>`). Additive — the
  core `sweep.sh` is unchanged (still curl-only, fast, whole-shortlist). Born out of
  the Dropeye naming session: the follow-up "trademark search" was listed but not
  actionable, and probing the registers proved they can't be curled — TMview is
  network-blocked (HTTP 000), WIPO branddb's API needs an ALTCHA proof-of-work + auth
  token (403/401), EUIPO/BOIP are SPA+auth. A real browser solves the ALTCHA
  automatically, so the new script drives the `sitescope` CDP browser through the WIPO
...
- Verified on `dropeye`: one WIPO hit — a French national mark, classes 9/35/38/41/42/45,
  **Expired 2024-02-07** → no live Benelux/EU mark, name clear. See the
  `trademark-clearance-route-benelux-eu` learning note for the full method.
