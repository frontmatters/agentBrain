...
name: cro-optimize
description: >-
  Conversion-rate optimization for landing/marketing/product pages: benefit-led
  CTAs, social proof, trust microcopy, incentives, objection-handling FAQ,
  form/funnel design, A/B testing, statistical significance. Use whenever the
  user wants to improve conversions, lift signups/sales, optimize a landing or
  pricing page, reduce friction/bounce, add trust or social proof, or asks "why
  isn't this converting" — even if they don't say "CRO". Applies accessibility-
  safe patterns (a11y and conversion are allies, not opposites).
---

<!-- PROVENANCE: vendored + sanitized 2026-07-26 from majiayu000/claude-skill-registry
     (skills/marketing/marketing-cro). Audited: markdown + metadata only, no scripts,
     no eval/exec/network/install-hooks. Adapted to agentBrain conventions and
     extended with the a11y×CRO principle below. Owned and maintained here — not a
     live dependency on the upstream repo. -->

## a11y × CRO are allies (agentBrain addition — proven)

Before the playbook below: conversion wins and accessibility wins usually point
the *same way*. A timid "link"-style CTA is both a conversion leak (too subtle)
and a contrast fail (~3:1) — **filling it fixes both at once**. Keep every CRO
lever accessible: benefit-CTA with large/bold text, trust microcopy that passes
AA, social-proof that isn't colour-only, and never trade WCAG-AA for a
"punchier" look. Measure an accent colour used as *small text* separately — a
brand accent fine as a button *fill* often fails as text. Prove with axe, don't
eyeball. See [[technique-transplant]].

## Pick the surface, then read its playbook

CRO is not one thing — each conversion **surface** has its own levers. Identify
the surface the user is optimizing, then read the matching deep playbook in
`references/` (each is a vendored, sanitized per-surface guide). The OS below is
the shared method; the references are where the specifics live.

| Surface the user is on | Read |
|---|---|
| Landing / marketing / home / product page | `references/landing-and-page.md` |
| A form (checkout, contact, application) | `references/forms.md` |
| Signup / registration / onboarding flow | `references/signup-and-onboarding.md` |
| Popup, modal, exit-intent, paywall, upgrade prompt | `references/popups-and-paywalls.md` |
| Pricing page / plan selection | `references/pricing.md` |
| Any headline / CTA / body copy | `references/conversion-copy.md` |
| Setting up / reading an experiment | `references/ab-testing.md` |

Always run the surface playbook **through** the a11y × CRO principle above — a
lever that lifts conversion but fails WCAG-AA is not a win.
