...
`~/agentBrain/vault/projects/<projectname>/index.md` with:
...
- One insight per file (`vault/learnings/<slug>.md`).
- File created via `bash ~/agentBrain/scripts/new-note.sh learning vault/learnings/<slug> "<title>"` for correct UUID5 + frontmatter.
...
\`\`\`
Read ~/agentBrain/vault/projects/<projectname>/index.md and pick up
...
\`\`\`
Unpark <projectname> via ~/agentBrain/vault/projects/<projectname>/index.md — execute the backlog.
\`\`\`

### Cross-agent (other model)

\`\`\`
Read ~/agentBrain/vault/projects/<projectname>/index.md carefully.
...
```
Parked:
- Project: ~/agentBrain/vault/projects/foo-skill/index.md
- Insights:
  - ~/agentBrain/vault/learnings/foo-edge-case.md
  - ~/agentBrain/vault/learnings/bar-pattern.md

Unpark prompt:
> Read ~/agentBrain/vault/projects/foo-skill/index.md and pick up
...
- **Mixing insights into the project document.** Insights belong in
  `vault/learnings/` so they are cross-project findable via tags.
