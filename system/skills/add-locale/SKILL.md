...
Add a new language to `scripts/lib/_strings.sh` so the localized surfaces appear in the user's preferred language. Currently localized: `scripts/selftest.sh`, `scripts/checks/check-agnostic.sh`, and the two addon installers that source `_strings.sh` (`session-journal`, `claude-memory-redirect`). Other scripts (doctor, setup) are English-only.
...
```bash
AGENTBRAIN_LOCALE=<code> bash ~/agentBrain/scripts/selftest.sh --only=claude-code
```

All checks should still pass (0 failed), with section headers in the new language.
