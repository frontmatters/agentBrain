...
- `--space <slug>` — opt-in view of a single space's own learnings
  (`vault/spaces/<slug>/learnings`) instead of the default view. Space
...
- `Read ~/agentBrain/vault/learnings/<slug>.md` — inspect a learning
...
The default view globs `vault/learnings/*.md` only, so space content
(`vault/spaces/<slug>/learnings/`) never appears unless `--space <slug>` is
