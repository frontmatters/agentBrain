...
name: config
description: >-
  Inspect and modify agentBrain configuration post-onboarding. Focus-based skill — walk-all (default) or one scope (locale, addons, hooks, preferences, shell-rc). Read-mostly: shows current state and routes mutations to the right tool rather than editing config directly.
...
Run `bash ~/agentBrain/scripts/checks/doctor.sh --summary --with-selftest` for a deeper
...
For each scope under `vault/preferences/`:

```bash
for scope in personal organization team; do
    dir="$HOME/agentBrain/vault/preferences/$scope"
