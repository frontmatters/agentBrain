...
resources:
  ...
  - vault/preferences/team/
---

# Grill Me

Extract tacit knowledge from the user via a relentless but focused interview.
The output lands in `vault/preferences/` and/or a dedicated brain note.
...
**Always write to:**
- `vault/preferences/personal/<topic>.md` — for personal facts and preferences
  (create or update; never overwrite content the user explicitly set)

**Optionally write to:**
- `vault/preferences/organization/<topic>.md` — if the user revealed org-level context
- `vault/preferences/team/<topic>.md` — if team agreements emerged
...
Generate UUID5: `bash scripts/uuid5-gen.sh "vault/preferences/personal/<filename-no-ext>"`
...
- `vault/projects/<name>/context.md` (for project-specific context)
- `vault/learnings/<topic>.md` (for a durable insight or pattern)

## Rules

- Never ask about credentials, passwords, or API keys — redirect to `vault/security/`
