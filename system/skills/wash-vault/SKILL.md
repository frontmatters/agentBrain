...
1. **filename-unsafe-chars** (runs first): file basename contains characters
   ...
   `check-vault-content` afterwards to detect orphaned references.
...
2. **no-frontmatter**: file has no YAML frontmatter at all. Fix: prepend a
   ...
   top-level folder under `vault/` (e.g. `learnings/`→`learning`,
...
The exempt-list mirrors `check-vault-content.sh` (machine-generated buckets:
...
| `--scope PATH` | Limit to one or more paths under `vault/`. Default: all of `vault/`. |
...
bash ~/agentBrain/scripts/checks/check-vault-content.sh
...
- **Wiki-link rewriting**: out of scope; the filename-rename rule may orphan
  internal references — detect via `check-vault-content` after a rename pass.

## Safety

- `--fix` is opt-in; dry-run is default.
- Each rule is independent — `--rules id-mismatch` runs only that rule.
- All mutations stay within `vault/` (never touches `system/`).
