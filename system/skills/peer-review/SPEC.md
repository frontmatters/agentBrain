...
**Skill location**: `$AGENTBRAIN_DIR/vault/skills/peer-review/`
...
- Optional archive of review under `vault/reviews/<timestamp>-<doc>-by-<reviewer>.md`.
...
| `--archive` | Save review **response + metadata** (not embedded doc-content) to `vault/reviews/<ts>-<doc>-by-<reviewer>.md`. | off |
...
```bash
peer-review ~/agentBrain/vault/skills/promote/SPEC.md
...
Sourceable bash file at `$AGENTBRAIN_DIR/vault/skills/peer-review/agents.sh`. **No JSON parser, no shell-string construction with user content.** Each agent gets its own bash function; the script calls `invoke_<name> "$PROMPT" "$MODEL"` (where `$MODEL` may be empty to use the CLI's own default).
...
Sourceable bash file at `$AGENTBRAIN_DIR/vault/skills/peer-review/config.sh` (optional; sensible defaults if absent).
...
12. **Archive** if `--archive` flag or `ARCHIVE_DEFAULT=true` in config: write to `vault/reviews/<ts>-<basename>-by-<reviewer>.md` with **response + metadata only**. Metadata header includes:
    - `doc-path` (absolute)
...
| `--archive` but `vault/reviews/` doesn't exist | `mkdir -p` it. |
...
```
$AGENTBRAIN_DIR/vault/skills/peer-review/
...
└── ../../reviews/           # archived reviews land here (sibling under vault/)
                             # $AGENTBRAIN_DIR/vault/reviews/<ts>-<doc>-by-<reviewer>.md
```
...
8. **Archive**: `peer-review SPEC.md --archive` → file appears at `vault/reviews/<ts>-SPEC-by-pi.md` with metadata header.
...
2. **Where should `config.sh` live?** Inside `vault/skills/peer-review/` (git-tracked) or in `~/.config/peer-review.sh` (outside the brain, untracked)? Inside is simpler (one location) but commits defaults. Recommendation: inside for v1; revisit if a privacy concern surfaces.
