---
name: peer-review
description: "Async cross-agent review of any document via the agentBrain event-bus. Emits peer-review.review.requested; any consumer agent (running as listener with any LLM backend) emits peer-review.review.completed. Caller can return immediately and poll later, or --wait for the first verdict. Review backend follows the cascaded config (system/security-policy.md): explicit --llm, then PEER_REVIEW_DEFAULT_LLM, then vault/config/llm.json, then a genuinely local ollama model; no shipped cloud default, cloud specs print their destination. Triggers: 'peer-review', 'let X review', 'second opinion', 'external review', 'have another model check this'."
---
...
| **archive** | `--archive <event_id>` | render a completed event to `vault/reviews/` |

## LLM backends (consume mode)

Default: cascaded config (system/security-policy.md). Explicit pins:

| Spec | Notes |
|---|---|
| `ollama-cloud:gpt-oss:20b-cloud` | ollama cloud — fast, free, no rate-limit; data-exit: documents leave the machine |
| `ollama-cloud:gpt-oss:120b-cloud` | deeper review, ~50s, found HIGHs that 20b missed; data-exit |
| `ollama:<local-model>` | local Ollama (private, slow without GPU); remote hosts via OLLAMA_HOST |
| `gemini` | gemini CLI (rate-limited as of 2026-05) |
| `echo` | fixed stub for pipeline-testing |

Without a pin the consumer resolves `vault/config/llm.json` (mapped to
`ollama:<model>`, `OLLAMA_HOST` for remote hosts) and then the first genuinely
local ollama model. Nothing configured: the consumer refuses with instructions
(exit 4). `:cloud` models are never auto-selected.
...
## Second mechanism: subagent-peers (inline, same-session)

The event-bus path above reviews a **document** with an **external model**. There
is a second, complementary mechanism: spawn 2–3 **parallel read-only subagents as
peers**, each with its own lens, when the review target is **live code/UX in a
repo** — they can grep, read files and cross-check claims against the source,
which an external model reviewing a pasted document cannot. Choose by target:

| Target | Mechanism |
|---|---|
| Spec/plan/design-doc; cross-model second opinion | event-bus (`bin/peer-review`) |
| Code, UI-flows, "staat X op de juiste plek?", workflow-kritiek | subagent-peers |

### Prompt-contract for subagent-peers (each rule earned by a real failure)

1. **One explicit lens per peer, lenses orthogonal** (e.g. structuur-vs-mock +
   terminologie; grief-UX + volledigheid + techniek). Convergent findings across
   orthogonal lenses are the strongest signal; identical lenses only add noise.
2. **Full context in the prompt**: repo path, what the thing is, exact file and
   spec/mock paths. Peers inherit nothing from the calling session.
3. **Demand a self-contained final report**: "werk dit ZELF volledig af in deze
   ene run — geen vervolgvragen, geen eigen achtergrond-agents; lever het
   volledige rapport als afsluitend bericht." (Failure 2026-07-19: a peer ended
   with "agents are still processing… what next?" — an unusable non-answer.)
4. **Read-only, always**: end with "GEEN code wijzigen". Reviewing peers that
   edit create merge-hazards with the caller's own work in the shared tree.
5. **Fix the report shape**: tabel per bevinding, top-N aanbevelingen met
   impact-schatting (klein/medium), plus an explicit "wat NIET aanraken"-lijst —
   that last section is what protects good existing choices from churn.

### Re-evaluation applies here identically

The §Post-review re-evaluation protocol below is mechanism-independent. Fresh
evidence (2026-07-19/20, subagent-peers on a client repo): two confident
"MEDIUM/HIGH" claims — "security-hardening ronde 1 is alleen ontwerp" and
"familie-tekstbewerking ontbreekt end-to-end" — were both **false positives**,
each refuted in under a minute with one grep (the hardening code + tests
existed; the PATCH-endpoint + UI existed). A third claim from the same round
("tweede contactpersoon ontbreekt") verified TRUE and was built. Verify first,
classify, then act.
...
- `vault/reviews/` — archived verdicts
