...
5. **Skills audit (keep/merge/rewrite/delete)**
   - Get usage evidence first: `bash scripts/flow/session-digest.sh --days 30 --format md`
     and read the "Skill usage (aggregated)" section. Skills the Skill tool never
     invoked in 30 days are candidates for merge/delete (note: usage data only
     covers Skill-tool invocations, not manual reads).
   - Inventory own skills: `system/skills/*/SKILL.md`, `vault/skills/*/SKILL.md`,
     and `~/.claude/skills/` (symlinks resolve to the former two).
   - Verdict per own skill: **keep / merge / rewrite / delete**, each with a one-line reason.
     - merge: overlapping purpose with another skill (name the target).
     - rewrite: triggers poorly (description mismatch) or body outdated.
     - delete: superseded, unused, and no unique knowledge.
   - Plugin skills (superpowers, pr-review-toolkit, ...): verdict **keep / disable** only,
     plus overlap warnings where an own skill duplicates a plugin skill.
   - Report only — never mutate skills during the retro; mutations follow after user review.

6. **Inspect memory hygiene and quality signals**
   - Run `bash scripts/checks/check-brain-review.sh` or `/brain-review`.
...
7. **Inspect the development backlog**
   - Review `vault/backlog/` titles and the most relevant current design notes.
   - Identify overlap, abandoned ideas, repeated plans, and the top 3 active bets.

8. **Make the analogy explicit**
   - Map findings to:
...
9. **Write a private retro report**
   - Save to `vault/reviews/brain-retro-YYYYMMDD.md` unless the user asks for chat-only output.
   - Include frontmatter:
     ```yaml
     ---
     date: YYYY-MM-DD
     ...
     ---
     ```
   - Generate UUID5 with `scripts/uuid5-gen.sh "vault/reviews/brain-retro-YYYYMMDD"`.
...
| Skill | Source | Usage (30d) | Verdict | Reason |
|---|---|---|---|---|

- Plugin overlap warnings:
...
- Keep the report private under `vault/` because it reflects real usage and priorities.
