...
write_doc() {
    ...
    local rel="vault/research/repo-distill/$SLUG/$stem"
    local uid
...
if [ -n "$REPO_PATH" ]; then
    sed -i.bak \
        -e "s|- Next action|- Next action: run \`bash scripts/scanman/scanman-scan.sh <repo-path> $SLUG\` and then manually enrich the generated docs|" \
        "$INDEX"
else
    sed -i.bak \
