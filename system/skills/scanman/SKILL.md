...
resources:
  ...
  - scripts/scanman/scanman-init.sh
  - scripts/scanman/scanman-scan.sh
  - scripts/scanman/scanman-validate.sh
---
...
Scanman's completion claim is **not** a judgment call. It is gated by `scripts/scanman/scanman-validate.sh`, which any agent on any platform can execute.

### The loop the agent MUST follow

```text
1. bootstrap     → bash scripts/scanman/scanman-scan.sh <repo-path> <slug>
2. enrich        → agent fills 01/02/03/04/05 with verified-first content
3. validate      → bash scripts/scanman/scanman-validate.sh <workspace-dir>
...
```text
vault/research/repo-distill/<repo-slug>/
├── index.md
...
Optional helpers:
- `bash scripts/scanman/scanman-init.sh <repo-slug> [repo-path] [goal...]`
- `bash scripts/scanman/scanman-scan.sh <repo-path> [repo-slug]`

Mandatory gate (see "Validation Gate" above):
- `bash scripts/scanman/scanman-validate.sh <workspace-dir>`
...
- `system/skills/scanman/VERSION`

Store project-specific analysis under `vault/research/repo-distill/<repo-slug>/` by default.
Only put it under `vault/projects/[name]/` when the repo analysis is directly part of an ongoing project record.
...
- Keep repo-specific facts in `vault/`

## Enrichment Model
- Keep one canonical doc set per repo under `vault/research/repo-distill/<repo-slug>/`
...
- [ ] `05-redesign-v1.md` is materially filled
- [ ] Remaining unknowns are explicit and bounded
- [ ] **`bash scripts/scanman/scanman-validate.sh <workspace-dir>` exits 0** (mandatory gate)
...
Or via the bash skill scripts: `bash scripts/scanman/scanman-init.sh --mode=reproduction-spec <slug> <repo-path>`.
...
### Commands
- Show current version: `bash scripts/scanman/scanman-bump-version.sh --show`
- Patch bump in `0.0.x`: `bash scripts/scanman/scanman-bump-version.sh patch`
- Validate release metadata: `bash scripts/scanman/scanman-release.sh check`
- Build a portable bundle: `bash scripts/scanman/scanman-build-release.sh`
