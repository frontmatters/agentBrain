...
- **Project number from /list-parks**: user types `/unpark 3` or similar
  → resolve via the cached index file at `~/agentBrain/vault/.parks-index.json`
...
- **Sanity check**: `vault/projects/<projectname>/index.md` exists. If not
...
```bash
{
  ...
  printf '| --- | --- | --- | --- | --- | --- |\n'
  for f in "$HOME"/agentBrain/vault/projects/*/index.md; do
    [ -f "$f" ] || continue
...
1. Read `~/agentBrain/vault/.parks-index.json`. If missing or stale
...
```bash
INDEX="$HOME/agentBrain/vault/.parks-index.json"
N=3
...
```
Read ~/agentBrain/vault/projects/<projectname>/index.md
```
...
Update `vault/projects/<projectname>/index.md` `## Progress` with one
line:
...
~/agentBrain/vault/projects/promote-demote-skill/index.md — execute
the backlog.
```
...
- **Cross-machine unpark**: if the project was parked on another machine,
  first run `gitea-sync` or equivalent so `vault/` is up to date.
