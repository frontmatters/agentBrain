...
| `/brain-recall` | Restore a forgotten note from `vault/.trash/forget/` | `system/skills/brain-recall/SKILL.md` |
...
| `/capture-tool-info` | Capture tool/auth/service info into `vault/` | `system/skills/capture-tool-info/SKILL.md` |
...
| `/project-update` | Create/update a project note in `vault/projects/` | `system/skills/project-update/SKILL.md` |
...
| `/brain-cmdb` | Query the CMDB (devices + integrations + typed relations); regenerate ## Relations | `system/skills/brain-cmdb/SKILL.md` |
| `/brain-ops` | Query + scaffold the ops-records layer (incident/problem/known-error/postmortem), CI-anchored | `system/skills/brain-ops/SKILL.md` |
...
| `/save-learning` | Save a real insight to `vault/learnings/` | `system/skills/save-learning/SKILL.md` |
...
Spaces are sealed owner compartments under `vault/spaces/<slug>/` (see `system/rules.md` → *Spaces / ownership*, and the full workflow in `docs/spaces.md`). Writes route into a space **per-write via context inference** (`system/lib/context.sh`) — the CWD's code-root, `AGENTBRAIN_CONTEXT=<slug>`, or an explicit `--context <slug>` on `new-note`. Create a space with `scripts/new-space.sh`; seal + back it up to its own private remote with `scripts/sync/sync-space.sh`. The old `active-space` session mode / `.active-space` marker is **decommissioned**.
...
`argument-hint`, `user-invocable`, `resources`. `scripts/setup/setup-skills.sh` installs every
...
machine via `vault/addons/<id>/enabled`. Manage with `scripts/addons.sh`
