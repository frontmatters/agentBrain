...
| Private | `vault/` → `~/.agentBrain/vault` | your notes, projects, learnings | its own private repo |
| **Shared** | `shared/` → `~/.agentBrain/shared` | knowledge you share with others | **its own scope repo** |
| Working | `~/.agentBrain/workspace` | material that is not knowledge: third-party checkouts (`external/`), rebuildable output (`derived/`), throwaway files (`scratch/`) | no git, nothing validates it |

`shared/` is a top-level layer (a sibling of `vault/`, not nested under it) backed by a
separate git repo, so it can have different access rights than your private vault. It is
gitignored from the framework repo, just like `vault/`.

## Set up a shared scope

```bash
# Bring your own remote (Gitea / GitHub / GitLab / ssh:// / a bare path)
bash scripts/setup/setup-shared-vault.sh --remote=ssh://host.tailnet/path/agentBrain-shared.git

# Or bootstrap a local bare repo (no server software needed; great for a tailnet)
bash scripts/setup/setup-shared-vault.sh --bootstrap
```
...
```bash
bash scripts/sync/sync-agentbrain-shared.sh "optional commit message"
```
...
`AGENTBRAIN_SHARED_HELPER`); set `AGENTBRAIN_SHARED_NO_TOKEN=1` for a vault/bare remote.

## Promote a note from private to shared

```bash
bash scripts/sync/promote-to-shared.sh learnings/my-note      # a single note
bash scripts/sync/promote-to-shared.sh references/some-folder # a whole folder
```

Promote moves the note(s) from `vault/` to `shared/`, **regenerates the path-derived UUID5**
...
`vault/` is private to you — a leaked secret stays on your disk. `shared/` is visible to
...
(`vault/specs/2026-06-21-shared-vault-{design,plan}.md`) — private-vault references,
