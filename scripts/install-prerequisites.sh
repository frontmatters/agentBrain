...
# skip themselves elsewhere (Linux gets jq etc. from apt in the preflight).
# These tools are useful regardless of which AI client you use.
# Pi itself is installed by scripts/configure-pi.sh.
# Idempotent — safe to re-run.
#
# Called by: scripts/bootstrap-macos.sh (macOS) and setup.sh's recommended-tools
...
ensure_brew_bundle() {
	...
	if ! command -v brew >/dev/null 2>&1; then
		...
		# Linux: no warn — these tools come from apt (installer preflight), not brew.
		return
	fi
