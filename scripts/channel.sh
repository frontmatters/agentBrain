...
PY
  ...
  local pick
  while true; do
    printf '   switch to [Enter = keep %s]: ' "$cur"
    IFS= read -r pick < /dev/tty || pick=""
    case "$pick" in
      "") echo "kept: $cur — nothing changed"; return 0 ;;
      1|stable)     main set stable; break ;;
      2|prerelease) main set prerelease; break ;;
      3|edge)       main set edge; break ;;
      *) echo "   (1-3, a channel name, or Enter to keep)" ;;
    esac
  done
  ...
  case "${1:-}" in
    ...
    set)
      ...
      case " ${CHANNELS[*]} " in *" $2 "*)
        ...
        # edge tracks a moving branch and has no tag to resolve — in tag-mode
        # that config combo is dead on arrival (brain update rejects it). Flip
        # the mode along instead of leaving a broken state for the user to debug.
        if [ "$2" = "edge" ] && [ "$(cfg_get mode)" = "tag" ]; then
          cfg_set mode branch; echo "mode -> branch (edge tracks a branch, not a tag)"
        fi ;;
