...
# shellcheck disable=SC2088  # the strings below are shown to the user; a literal ~ is the point
...
_t_nl() {
	case "$1" in
		...
		selftest.sj.config_seeded)          echo "journal-config.json geseed in vault/" ;;
		selftest.sj.config_missing)         echo "journal-config.json mist in vault/sessions/" ;;
		...
		selftest.cmr.config_seeded)         echo "claude-redirect-config.json geseed in vault/" ;;
		selftest.cmr.config_missing)        echo "claude-redirect-config.json mist in vault/memories/" ;;
...
_t_en() {
	case "$1" in
		...
		selftest.sj.config_seeded)          echo "journal-config.json seeded in vault/" ;;
		selftest.sj.config_missing)         echo "journal-config.json missing in vault/sessions/" ;;
		...
		selftest.cmr.config_seeded)         echo "claude-redirect-config.json seeded in vault/" ;;
		selftest.cmr.config_missing)        echo "claude-redirect-config.json missing in vault/memories/" ;;
