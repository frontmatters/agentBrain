...
#
# Usage:
#   bash scripts/setup/setup-launchd-loop.sh                # install + bootstrap
#   bash scripts/setup/setup-launchd-loop.sh --uninstall    # remove
#   bash scripts/setup/setup-launchd-loop.sh --kickstart    # install + fire once now
#
...
LOGS_DIR="${VAULT}/vault/logs"  # in-brain logs (per connector model — brain content lives in brain)
