...
# The vault is vault/, a symlink that setup-vault.sh points outside the
...
if [ ! -L "${VAULT}/vault" ]; then
	echo -e "${YELLOW}!${NC} ${VAULT}/vault is not a symlink; run scripts/setup/setup-vault.sh first." >&2
	exit 1
fi
LOCAL_DIRS=(
	"vault/projects"
