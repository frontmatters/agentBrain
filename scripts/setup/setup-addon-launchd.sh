...
#
# Usage:
#   bash scripts/setup/setup-addon-launchd.sh install <addon-id>
#   bash scripts/setup/setup-addon-launchd.sh uninstall <addon-id>
#   bash scripts/setup/setup-addon-launchd.sh kickstart <addon-id>
#   bash scripts/setup/setup-addon-launchd.sh status <addon-id>
#
...
VAULT="${VAULT:-$(cd "$(dirname "$(realpath "${BASH_SOURCE[0]}")")/../.." && pwd)}"
...
[[ -n "$ADDON_DIR" ]] || { echo "setup-addon-launchd: no manifest for '$ID' in vault/ or system/addons" >&2; exit 1; }
