...
# was assumed by the validators (check-anchors.sh, check-vault-content.sh) but created
...
if [ -L "$LINK" ]; then
	...
	# A real local/ directory NEXT TO the vault link is a leftover of an earlier
	# lineage (the vault's old name, from before the rename). It is not the vault
	# and setup must not merge it blindly; it is also not framework, and left in
	# place it turned check-readmes and check-frontmatter red on two machines.
	# Set it aside beside the checkout, never delete it, and say so.
	if [ "$LINK" = "$ALIAS" ] && [ -d "$LOCAL" ] && [ ! -L "$LOCAL" ]; then
		aside="$(cd "$VAULT/.." && pwd)/agentBrain-local-leftover-$(date +%Y%m%d-%H%M%S)"
		mv "$LOCAL" "$aside"
		echo -e "${YELLOW}Moved${NC} a leftover real local/ directory out of the checkout: $aside"
		echo "  Its notes are not in the vault. Review them; move what you keep into the vault with new-note.sh."
	fi
	exit 0
fi
