...
if [ -d "${VAULT}/vault" ]; then
	...
	if [ -n "$NS" ] && [ ! -s "$BACKUP" ]; then
		...
		# Write-once. The backup exists so a changed brain.json is noticed; a setup
		# that overwrote it on mismatch (a fresh checkout mounted on a populated
		# vault, or a sandbox that reached the real vault) erased the evidence.
		echo -e "${YELLOW}!${NC} brain.json namespace differs from vault/brain-namespace.backup; left the backup alone (check-anchors reports it)." >&2
	fi
fi
