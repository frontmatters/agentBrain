...
--staged | staged)
	...
	hits=$(git grep --cached -n -I -i -E "$PATTERN" -- . ':(exclude)vault/**' || true)
	;;
tracked | --tracked)
	hits=$(git grep -n -I -i -E "$PATTERN" -- . ':(exclude)vault/**' || true)
	;;
--dir)
...
if [[ -f "$LOCAL_DENYLIST" ]]; then
	while IFS= read -r deny_pattern; do
		...
		case "$MODE" in
		--staged | staged)
			extra_hits=$(git grep --cached -n -I -i -E "$deny_pattern" -- . ':(exclude)vault/**' || true)
			;;
		--dir)
			extra_hits=$(grep -rn -I -i -E --exclude-dir=.git "$deny_pattern" "$SCAN_DIR" 2>/dev/null || true)
			;;
		*)
			extra_hits=$(git grep -n -I -i -E "$deny_pattern" -- . ':(exclude)vault/**' || true)
			;;
		esac
