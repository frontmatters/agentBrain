...
run_claude_code() {
	...
	if [[ -f "$BRAIN_ROOT/vault/sessions/journal-config.json" ]]; then
	...
	if [[ -f "$BRAIN_ROOT/vault/sessions/session-journal.md" ]]; then
		local mt_human
		mt_human="$(stat -f '%Sm' -t '%Y-%m-%d %H:%M' "$BRAIN_ROOT/vault/sessions/session-journal.md" 2>/dev/null \
		         || stat -c '%y' "$BRAIN_ROOT/vault/sessions/session-journal.md" 2>/dev/null)"
	...
	if [[ "$SJ_TEST_OUT" == *"journal updated"* ]] || grep -q "(auto: selftest)" "$BRAIN_ROOT/vault/sessions/session-journal.md" 2>/dev/null; then
	...
	if [[ -f "$BRAIN_ROOT/vault/memories/claude-redirect-config.json" ]]; then
		ok "$(t selftest.cmr.config_seeded)"
		local mode
		mode="$(python3 -c "
import json
print(json.load(open('$BRAIN_ROOT/vault/memories/claude-redirect-config.json')).get('mode','?'))
" 2>/dev/null || echo "?")"
