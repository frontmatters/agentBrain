...
run_generic() {
	...
	if [[ -x "$BRAIN_ROOT/scripts/hooks/claude-code-validate-note-id-hook.sh" ]]; then
	...
	if [[ -x "$BRAIN_ROOT/scripts/checks/check-frontmatter.sh" ]]; then
		local fm_out; fm_out="$(bash "$BRAIN_ROOT/scripts/checks/check-frontmatter.sh" 2>&1 || true)"
	...
	if [[ -x "$BRAIN_ROOT/scripts/checks/check-session-schema.sh" ]]; then
		local schema_out; schema_out="$(bash "$BRAIN_ROOT/scripts/checks/check-session-schema.sh" 2>&1 || true)"
