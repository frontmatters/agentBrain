...
detect_copilot_cli() {
	command -v copilot &>/dev/null || [[ -d "${AGENTBRAIN_HOME:-$HOME}/.copilot" ]]
}

run_copilot_cli() {
	hdr "GitHub Copilot CLI"

	local POINTER="${AGENTBRAIN_HOME:-$HOME}/.copilot/copilot-instructions.md"
	if [[ -f "$POINTER" ]]; then
	...
	local skills_dir="${AGENTBRAIN_HOME:-$HOME}/.copilot/skills"
