...
#
# Rule: non-English user-visible strings (section headers, TODO placeholders, LLM
# prompt language) must be centralized in locale.ts, not scattered across source
# files. The detectors below scan for Dutch — the founding locale and the only
# non-English one shipped so far; extend them when a new locale lands.
