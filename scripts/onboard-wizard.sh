...
PREFS = f"{VAULT}/vault/preferences/personal"
...
PROMPT_HELPER = os.path.join(VAULT, "scripts", "installer", "prompt-helper.sh")

def shared_prompt(mode, question, opts, default="1"):
    if DEFAULTS or not TTY or os.environ.get("AB_WIZARD_PLAIN") == "1":
        return None
    try:
        env = os.environ.copy()
        env["AB_PROMPT_HELPER_STANDALONE"] = "1"
        # Rows carry the value plus its description so nothing is lost.
        rows = [o["value"] + (f" \u2014 {o['desc']}" if o.get("desc") else "") for o in opts]
        cmd = ["bash", PROMPT_HELPER, mode]
        if default:
            cmd += ["--default", default]
        cmd += [question] + rows
        result = subprocess.run(
            cmd, stdin=TTY, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL,
...
# An option can declare a requirement it only makes sense under. The one that
# exists today is "insider": the edge channel offers every change immediately,
# which is only true for someone following the development checkout. Installed
# from the public snapshot, edge means "the last deploy" — three weeks old at
# the time of writing, under a label promising the opposite.
#
# Derived, not declared: the checkout's own origin says where it came from, and
# a recorded flag would drift the moment someone re-points the remote.
def _is_insider():
    import subprocess
    try:
        url = subprocess.run(["git", "-C", VAULT, "remote", "get-url", "origin"],
                             capture_output=True, text=True, timeout=5).stdout.strip()
    except (OSError, subprocess.SubprocessError):
        return False
    if not url:
        return False
    # A public host serves the sanitized snapshot; anything else (a LAN git://
    # bundle remote, a private Gitea) is a development checkout.
    return not any(h in url for h in ("github.com", "gitlab.com", "codeberg.org"))


def _available(opts):
    """Options whose stated requirement holds here."""
    out = []
    for o in opts:
        req = o.get("requires")
        if req == "insider" and not _is_insider():
            continue
        out.append(o)
    return out or opts          # never present an empty menu


def ask_choice(name, field, question, detected=None):
    opts = _available(field["options"])
    di = _default_idx(opts, detected)
    shared = shared_prompt("select", question, opts, di)
    if shared is not None and shared.isdigit() and 0 <= int(shared) < len(opts):
        pick = int(shared)
...
def ask_multi(name, field, question, detected=None):
    ...
    if not DEFAULTS and _checkbox_capable():
        try:
            on_rows = [str(i + 1) for i, o in enumerate(opts) if o.get("on")]
            pre = " ".join(on_rows) or str(di + 1)
            shared = shared_prompt("multi", question, opts, pre)
            if shared is None:
                raise RuntimeError("shared prompt unavailable")
            picks = [opts[int(i)]["value"] for i in shared.split() if i.isdigit() and 0 <= int(i) < len(opts)]
...
def full_flow():
    ...
    _det_ed = detect_editor()
    _ed_q = "Editor? (detected)" if _det_ed else "Editor?"
    answers["editor"] = ask_field("editor", _ed_q, _det_ed or "VS Code")
...
write_pref("tech-stack.md", "Tech Stack", [
    ...
    "- Infrastructure: (fill in later — /onboard asks this)",
])
write_pref("workflow.md", "Workflow", [
    f"- Autonomy: {answers['autonomy']}",
    "- Projects location: (fill in later — /onboard asks this)",
    "- Shell/tooling preferences: (fill in later — /onboard asks this)",
])
write_pref("design-philosophy.md", "Design Philosophy", [
    f"- Theme: {answers['design']}",
    "- Visual style: (fill in later — /onboard asks this)",
    "- Icons/typography: (fill in later — /onboard asks this)",
])
write_pref("decision-making.md", "Decision Making", [
    f"- Approach: {answers['decision']}",
    "- Build vs buy: (fill in later — /onboard asks this)",
    "- Backwards compatibility vs clean breaks: (fill in later — /onboard asks this)",
])
write_pref("identity.md", "Identity", [
    ...
    "- Additional identities: (fill in later — /onboard asks this)",
])
...
subprocess.run(["bash", f"{VAULT}/scripts/channel.sh", "set", answers["channel"]],
               capture_output=True)
import pathlib
cfg_p = pathlib.Path(f"{VAULT}/vault/update/config.json")
