...
NOTE_DIR="${VAULT}/vault/daily-notes"
...
UUID=$(bash "${VAULT}/scripts/uuid5-gen.sh" "vault/daily-notes/${TODAY}") || {
...
if [[ -f "${TEMPLATE}" ]]; then
	...
	# UUID5 is deterministic per vault-relative path (matches uuid5-gen.sh + check-vault-content).
