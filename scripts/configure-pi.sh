...
maybe_pi_keychain_setup() {
	...
	if [[ -f "$PI_CONFIG_DIR/auth.json" ]]; then
		warn "Pi auth.json found but no pi-agent keychain. To migrate a key into the macOS Keychain: security add-generic-password -a \"\$USER\" -s pi-<provider>-access -w '<api-key>' ~/Library/Keychains/pi-agent.keychain-db (create the keychain first with: security create-keychain pi-agent.keychain)."
	else
