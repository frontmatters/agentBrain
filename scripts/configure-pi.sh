...
install_pi_config() {
	...
	skilllib_link_standalone_skills "$PI_CONFIG_DIR/skills" "$PI_SRC/local/skills" "local/skills" "$PI_SRC"
...
maybe_pi_keychain_setup() {
	...
	if [[ -f "$PI_CONFIG_DIR/auth.json" ]]; then
		warn "Pi auth.json found but no pi-agent keychain — API keys are sitting in a plain file. Recommended: install the secrets-helper addon (bash scripts/addons.sh install secrets-helper) — /onboard offers this on macOS. Manual alternative: security create-keychain pi-agent.keychain, then security add-generic-password -a \"\$USER\" -s pi-<provider>-access -w '<api-key>' ~/Library/Keychains/pi-agent.keychain-db"
	else
