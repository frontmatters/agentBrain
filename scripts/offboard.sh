...
while [[ $# -gt 0 ]]; do
	...
	-h | --help)
		...
		echo "  --include-team          Include vault/preferences/team/ in the export"
		echo "  --include-organization  Include vault/preferences/organization/ in the export"
...
echo "Running doctor..."
(cd "$VAULT" && bash scripts/checks/doctor.sh --summary) || true
echo ""
...
PREF_COUNT=0
PERSONAL_PREFS_SRC="${VAULT}/vault/preferences/personal"
LEGACY_PREFS_SRC="${VAULT}/vault/preferences"
...
if [[ "$INCLUDE_TEAM" == true && -d "${VAULT}/vault/preferences/team" ]]; then
	mkdir -p "${EXPORT_DIR}/preferences/team"
	for f in "${VAULT}/vault/preferences/team"/*.md; do
		[[ -f "$f" ]] || continue
		BASE=$(basename "$f")
...
if [[ "$INCLUDE_ORG" == true && -d "${VAULT}/vault/preferences/organization" ]]; then
	mkdir -p "${EXPORT_DIR}/preferences/organization"
	for f in "${VAULT}/vault/preferences/organization"/*.md; do
		[[ -f "$f" ]] || continue
		BASE=$(basename "$f")
...
DAILY_SRC="${VAULT}/vault/daily-notes"
if [[ -d "$DAILY_SRC" ]]; then
	NOTE_COUNT=0
...
LEARNS_SRC="${VAULT}/vault/learnings"
...
for cfg in registries.json default-url; do
	if [[ -f "${VAULT}/vault/addons/${cfg}" ]]; then
		cp "${VAULT}/vault/addons/${cfg}" "${EXPORT_DIR}/config/${cfg}"
	fi
done
# Enabled addons, one id per line.
if [[ -d "${VAULT}/vault/addons" ]]; then
	for marker in "${VAULT}/vault/addons"/*/enabled; do
		[[ -f "$marker" ]] || continue
...
1. Install agentBrain on the new machine:
   ...
   cd ~/Developer/agentBrain && bash scripts/setup/setup.sh
   ```
...
if [[ -n "$GPG_MODE" ]]; then
	...
	echo ""
	echo "Note: export is NOT encrypted — it contains your whole vault."
