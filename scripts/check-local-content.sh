...
def walk_md(root):
    ...
    Prunes third-party/generated dirs (see _PRUNE_DIRS) in-place."""
...
def is_exempt(rel_path: str) -> bool:
    ...
    for pat in ("local/skills/*/SKILL.md",
                ...
                "local/metrics/**/*",
                "local/learnings/extracted/*",     # machine-generated by yt-digest learn; content-hash id strategy
                "local/learnings/extracted/**/*",
                "local/youtube-digest/*",       # transcripts + category index files (README/<category>.md)
                ...
                "local/analyses/**/*",
                "local/spaces/*/analyses/*",       # per-space uxray/sitescope analysis bundles — same machine-generated shape
                "local/spaces/*/analyses/**/*",
                "local/archive/*",                 # raw imported historical content (e.g. ChatGPT exports); not curated notes
                "local/archive/**/*",
                "local/legacy/*",                  # raw imported ClaudeBrain session logs — same category as archive/, not curated notes
                "local/legacy/**/*",
                "local/spaces/*/legacy/*",         # per-space imported session logs (malformed [[[ links, no frontmatter by design)
                "local/spaces/*/legacy/**/*",
                "local/tmp/*",                     # transient scratch space — not curated notes
                "local/tmp/**/*",
                "local/addons/*/finetune/*",       # core-bench: model-finetune training data (synthetic vault notes)
                "local/addons/*/finetune/**/*",
                "local/addons/*/runs/*",           # core-bench: benchmark run outputs — fixtures with deliberate placeholder ids
                "local/addons/*/runs/**/*",
                "local/skills/*/references/*",      # installed-skill internal reference docs — skill payload, not vault notes
                "local/skills/*/references/**/*",
                "local/skills/*-workspace/*",       # skill eval/benchmark workspaces (e.g. component-spec-reader) — machine-generated outputs
                "local/skills/*-workspace/**/*",
                "local/skills/*/templates/*",       # installed-skill template payload (e.g. brain-architect) — consumed by the skill, not vault notes
                "local/skills/*/templates/**/*",
                "local/tools/*",                    # tool-prototype workspaces (build scripts, HTML/JSON specs, working docs) — not curated notes
                "local/tools/**/*",
                "local/queue/index.md",             # queue.sh kanban board — tool-managed, no frontmatter by design
                "local/sessions/digests/*",         # session-digest.sh output — machine-generated, no frontmatter
                "local/sessions/digests/**/*",
                "local/reports/core-bench-*.md"):   # core-bench addon benchmark report — machine-generated
