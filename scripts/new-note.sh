...
case "$TYPE" in
	...
	project)   TAGS="[project]"   ; SOURCE="" ; STATUS_FIELD="status: active"
	           # One commented line, not three. Four of 123 project notes are
	           # products, so anything longer is boilerplate in 119 files nobody
	           # needs it in. It carries the question; the channel syntax lives in
	           # the pubcheck README, not in every note. No machine-specific paths
