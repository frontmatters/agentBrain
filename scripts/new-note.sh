...
# Active-space default: when no explicit --space was given but a space is active
# (env AGENTBRAIN_SPACE or the local/.active-space marker), write into it so notes
# land in the active compartment automatically. Explicit --space always wins; with
# no active space, behavior is unchanged. The resolved slug still flows through the
# path-escape guard below, so an invalid active slug fails just like an explicit one.
if [ "$SPACE_GIVEN" = "0" ]; then
	_NN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
	_ACTIVE_SPACE="$(bash "$_NN_DIR/active-space.sh" resolve 2>/dev/null || true)"
	if [ -n "${_ACTIVE_SPACE:-}" ]; then
		SPACE_SLUG="$_ACTIVE_SPACE"
		SPACE_GIVEN=1
	fi
fi
