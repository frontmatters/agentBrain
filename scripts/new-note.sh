...
Usage: $0 <type> <vault-relative-path-no-ext> [title]
  type: learning | project | backlog | feedback | reference | session | spec | task
...
# Guard: every note lives under local/ (the vault). A caller that omits the prefix
# (e.g. "learnings/foo" instead of "local/learnings/foo") would otherwise write to
# the CHECKOUT ROOT — outside the vault, git-untracked, and invisible to
# validate-note-id.sh (which only checks local/*), so the path-derived UUID5 silently
...
# local/spaces/ above, so this is a no-op for them.
case "$REL_PATH_NO_EXT" in
	local/*) : ;;
	*)
		echo "new-note: path '${REL_PATH_NO_EXT}' is not under local/ — auto-prefixing to 'local/${REL_PATH_NO_EXT}'" >&2
		REL_PATH_NO_EXT="local/${REL_PATH_NO_EXT}"
		;;
esac
...
case "$TYPE" in
	...
	task)      TAGS="[task]"      ; SOURCE="" ; STATUS_FIELD="status: pending" ;;
	...
	*) echo "Unknown type: $TYPE (use: learning|project|backlog|feedback|reference|session|spec|task; for a space use scripts/new-space.sh)" >&2; exit 2 ;;
esac
