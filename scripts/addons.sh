...
. "$ROOT_DIR/scripts/lib/platform.sh" 2>/dev/null || . "$(dirname "$0")/platform.sh"
...
brain_maintainer() {
	python3 -c 'import json,sys;print(json.load(open(sys.argv[1])).get("maintainer") or "unknown")' \
...
# Extract a subfield of the nested `deprecated:` block from a manifest/SKILL.md.
_dep_sub() { awk -v k="$2" '/^---[[:space:]]*$/{fm++;next} fm==1&&/^deprecated:[[:space:]]*$/{s=1;next} fm==1&&s&&/^[^[:space:]]/{s=0} fm==1&&s&&$0 ~ "^[[:space:]]+"k":"{sub("^[[:space:]]+"k":[[:space:]]*","");sub(/[[:space:]]*#.*$/,"");gsub(/"/,"");print;exit}' "$1"; }
...
cmd_status() {
	...
	local id m name state ver src upd best bver breg auth
	if [ "$remote" -eq 1 ]; then
		printf '%-22s %-9s %-10s %-8s %-16s %-22s %s\n' "ID" "VERSION" "STATE" "SOURCE" "AUTHOR" "UPDATE" "NAME"
	else
		printf '%-22s %-9s %-10s %-8s %-16s %s\n' "ID" "VERSION" "STATE" "SOURCE" "AUTHOR" "NAME"
	fi
	for id in $(all_addon_ids); do
		m="$(manifest_path "$id")"
		if [ -z "$(_field "$m" id)" ]; then
			echo "WARN: malformed manifest: $m — run scripts/checks/check-addons.sh" >&2
			continue
		fi
		name="$(_field "$m" name)"
		ver="$(_field "$m" version)"
		src="$(addon_source "$id")"
		# Attribution: an explicit `author:` in the manifest, else the first-party
		# default. Community/adopted addons stamp their own author; ours don't need to.
		auth="$(_field "$m" author)"; [ -n "$auth" ] || auth="$DEFAULT_MAINTAINER"
		...
		if [ "$remote" -eq 1 ]; then
			...
			printf '%-22s %-9s %-10s %-8s %-16s %-22s %s\n' "$id" "${ver:--}" "$state" "$src" "$auth" "$upd" "$name"
		else
			printf '%-22s %-9s %-10s %-8s %-16s %s\n' "$id" "${ver:--}" "$state" "$src" "$auth" "$name"
		fi
		# Deprecation notice: renamed/replaced/retired addons surface a pointer here.
		if grep -qE "^deprecated:[[:space:]]*$" "$m"; then
			local dreason drepl drem
			dreason="$(_dep_sub "$m" reason)"; drepl="$(_dep_sub "$m" replaced_by)"; drem="$(_dep_sub "$m" remove_after)"
			if [ -n "$drepl" ]; then
				printf '    \342\232\240 deprecated (%s) \342\206\222 use %s%s\n' "$dreason" "$drepl" "${drem:+, removed after $drem}"
			else
				printf '    \342\232\240 deprecated (%s)%s \342\200\224 no replacement\n' "$dreason" "${drem:+, removed after $drem}"
			fi
		fi
	done
}
...
sync_addon_skills() {
	...
	local err
	if ! err="$(ADDONS_STATE="$STATE" bash "$ROOT_DIR/scripts/setup/setup-skills.sh" sync-addons 2>&1 >/dev/null)"; then
...
cmd_enable() {
	...
	local _m _rtv _rt
	_m="$(manifest_path "$id")"
	_rtv="$(_field "$_m" runtime_requires)"
	for _rt in ${_rtv//,/ }; do
		offer_install "$_rt" || true   # soft: never block enable on a decline
	done
}
...
cmd_disable() {
	...
	if [ -f "$(manifest_path "$id" 2>/dev/null)" ] && has_schedule "$id" 2>/dev/null \
			&& [ "$(uname -s)" = "Darwin" ]; then
		bash "$ROOT_DIR/scripts/setup/setup-addon-launchd.sh" uninstall "$id" 2>/dev/null || true
	fi
}
...
cmd_privacy() {
	...
	ab_prompt_confirm --default no "Enable $id?" && return 0
	echo "Aborted."; return 3
}

run_install_cmd() {
	local id="$1" cmd="$2"
	[ -n "$cmd" ] && [ "$cmd" != "none" ] || return 0
...
_run_install_step() {
	...
	# `install: none` is the documented way to say "nothing to run" (goal,
	# lottie-animator, web-interface-guidelines). It was executed as a command.
	[ "$cmd" = "none" ] && cmd=""
	case "$method" in
		self)
			if [ -n "$cmd" ]; then run_install_cmd "$id" "$cmd"; fi ;;
		ai-driven)
			echo "ai-driven install: open the per-client INSTALL.md and follow it:"
			echo "  $cmd" ;;
		config-entry)
			echo "config-entry install: enabling the addon IS the config entry (enable flag + skill link); no separate installer runs."
			if [ -n "$cmd" ]; then run_install_cmd "$id" "$cmd"; fi ;;
...
download_addon() {
	...
	# Zip-slip guard: reject any entry with an absolute path or a `..` component
	# BEFORE extraction, so nothing can be written outside the temp dir.
	if unzip -Z1 "$zip" | grep -Eq '(^/|(^|/)\.\.(/|$))'; then
		echo "Refusing $id archive: entry with absolute path or '..' component (zip-slip)" >&2
		rm -rf "$tmp"; return 1
	fi
...
install_from_registry() {
	...
	# registry_candidates emits 7 TSV fields (…url sha256 author); read all 7 so the
	# trailing author never slurps into $sha (a 6-var read made sha="<sha>\t<author>"
	# → a false "sha256 mismatch" on every registry install). The 7th (author) is
	# discarded into _.
	IFS="$TAB" read -r reg id ver name url sha _ <<EOF2
$chosen
EOF2
...
cmd_update() {
	...
	chosen="$(printf '%s\n' "$cands" | sort -t"$TAB" -k3,3V | tail -1)"
	# 7 fields (…sha256 author) — read all 7 so author never slurps into $sha; the
	# trailing author is discarded into _.
	IFS="$TAB" read -r reg id ver name url sha _ <<EOF2
$chosen
EOF2
...
_schedule_install_prompt() {
	...
	if [ "${ADDONS_ASSUME_YES:-0}" = "1" ]; then
		...
		echo "    bash scripts/setup/setup-addon-launchd.sh install $id" >&2
		return 0
	else
		ab_prompt_confirm --default no "Install the launchd job now?" && ans=y || ans=n
	fi
	case "$ans" in
		y|Y|yes|YES)
			bash "$ROOT_DIR/scripts/setup/setup-addon-launchd.sh" install "$id"
			;;
		*)
			echo "  Skipped. Install later with:"
			echo "    bash scripts/setup/setup-addon-launchd.sh install $id"
			;;
	esac
}
...
_onboard_install_prompt() {
	...
	[ -n "$run" ] || return 0
	if [ -n "$req" ]; then
		if [ ! -f "$ROOT_DIR/scripts/lib/platform.sh" ]; then
	...
	if [ "${ADDONS_ASSUME_YES:-0}" = "1" ]; then
		...
		ab_prompt_confirm --default no "Run it now?" && ans=y || ans=n
	fi
	case "$ans" in
...
cmd_check() {
	...
	for _id in $(all_addon_ids); do
		...
		# Soft dependency check: warn (never fail) when a declared `requires:` add-on
		# is not enabled. Static validity of the ids is enforced by check-addons.sh.
		local req_val req
		req_val="$(_field "$m" requires)"
		if [ -n "$req_val" ]; then
			for req in ${req_val//,/ }; do
				is_enabled "$req" || echo "warn $id requires '$req' (not enabled)" >&2
			done
		fi
...
cmd_test() {
	...
	for _id in $(all_addon_ids); do
		...
		if ADDONS_CHECK_REGISTRY="$(dirname "$(addon_dir "$_id")")" bash scripts/checks/check-addons.sh "$id"; then
...
cmd_configure() {
	...
	if [ "$count" = "1" ]; then
		...
		_cfg_rows=(); while IFS= read -r _cfg_row; do [ -n "$_cfg_row" ] && _cfg_rows+=("$_cfg_row"); done <<< "$files"   # a read loop: bash 3.2 (macOS /bin/bash) has no array-from-lines builtin
		if ab_prompt_select --required "Which one to edit?" "${_cfg_rows[@]}"; then
			target="${_cfg_rows[$REPLY]}"
		else
...
cmd_clients() {
	...
	# mktemp + trap: never leave a half-written matrix behind in the registry
	# (same pattern as scripts/checks/*; an aborted run used to strand .clients.gen.<pid>).
	# A RETURN trap would outlive this function (bash keeps it for the caller's
	# return, where a local would be unbound), so use a global + EXIT trap.
	CLIENTS_TMP="$(mktemp "$REGISTRY/.clients.gen.XXXXXX")"
	trap 'rm -f "${CLIENTS_TMP:-}"' EXIT
...
cmd_new() {
	...
	for f in "$target/manifest.md" "$target/README.md"; do
		[ -f "$f" ] || continue
		sed -i.bak "s/your-addon-id/$id/g; s/Your Addon Name/$name/g; s|your-handle|$DEFAULT_MAINTAINER|g" "$f"
	done
	rm -f "$target"/*.bak
...
cmd_search() {
	need_jq || return 1
	local term="${1:-}"
	local id m ver name auth
	printf '%-22s %-9s %-12s %-16s %s\n' "ID" "VERSION" "SOURCE" "AUTHOR" "NAME"
	for id in $(all_addon_ids); do
		...
		name="$(_field "$m" name)"
		auth="$(_field "$m" author)"; [ -n "$auth" ] || auth="$DEFAULT_MAINTAINER"
		printf '%-22s %-9s %-12s %-16s %s\n' "$id" "${ver:--}" "$(addon_source "$id")" "$auth" "$name"
	done
	registry_candidates "" | {
		...
		END {
			for (i = 1; i <= NR; i++) {
				...
				auth = (f[7] == "" ? "unknown" : f[7])
				printf "%-22s %-9s %-12s %-16s %s%s\n", f[2], f[3], f[1], auth, f[4], mark
			}
		}'
}
