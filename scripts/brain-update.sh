...
# Hard time budget: an unreachable remote must never hang a session-start
# update check (git's own TCP timeout can take minutes; macOS has no timeout(1)).
...
if kill -0 "$_fpid" 2>/dev/null; then
  ...
  die "fetch from '$remote' timed out after 8s (unreachable?) — nothing changed"
fi
if ! wait "$_fpid"; then
