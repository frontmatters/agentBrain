...
if [ "$SESSION" -eq 1 ]; then
  # Mark the whole session-start check (and its --check/full subprocesses) as
  # automated: the fetch below then uses a tighter budget and fails SILENTLY on an
  # unreachable/private origin, so a session start never hangs or shows a LAN URL.
  export BRAIN_UPDATE_SESSION=1
  ...
  stamp="${AGENTBRAIN_DIR:-$HOME/agentBrain}/vault/update/.last-session-check"
  ...
  case "$m" in
    ...
    ask)
      ...
      if [ "$rc" -eq 10 ]; then
        ...
        if [ -t 0 ] && [ -t 1 ]; then            # interactive shell: ask the human directly
          if ab_prompt_confirm --default no "Update agentBrain now?"; then bash "$0" ${REPO:+--repo "$REPO"} || true; fi
    ...
    auto)
      ...
      auto_log="${AGENTBRAIN_DIR:-$HOME/agentBrain}/vault/update/last-auto.log"
...
c() { printf '\033[%sm' "$1"; }
ok()   { printf '%s✓%s %s\n' "$(c '1;32')" "$(c 0)" "$*"; }
# Progress/diagnostics go to stderr — NOT stdout. The session-start hook injects
# this script's stdout into the agent context, so a "fetching <remote-url> …" line
# on stdout would surface a private/LAN origin URL in the session. Keep stdout for
# results only (ok/the check verdict); routes the remote URL away from the session.
info() { printf '%s·%s %s\n' "$(c 2)" "$(c 0)" "$*" >&2; }
warn() { printf '%s⚠%s %s\n' "$(c '1;33')" "$(c 0)" "$*" >&2; }
die()  { printf '%s✗%s %s\n' "$(c '1;31')" "$(c 0)" "$*" >&2; exit 1; }
...
# The update gate checks HEALTH, not personalization: an un-onboarded machine
...
# What a channel resolves to is decided by the channel, not only by the mode.
# 'edge' is a branch by nature: it moves with every commit, so there is no tag
# for it to point at, in either mode. The mode governs how RELEASE channels
# (prerelease, stable) resolve. Deciding this once, here, keeps the two places
# below from drifting apart — they used to test "$mode" separately, and the tag
# lookup then rejected edge outright.
chan_is_branch=0
[ "$mode" = "branch" ] && chan_is_branch=1
[ "$channel_name" = "edge" ] && chan_is_branch=1
...
# Hard time budget: an unreachable remote must never hang a session-start update
# check (git's own TCP timeout can take minutes; macOS has no timeout(1)). In a
# session-context check (BRAIN_UPDATE_SESSION=1) the budget is tighter and any
# failure is SILENT (exit 0, no message) — a private/unreachable origin must never
# surface a timeout or a LAN URL at session start.
_session="${BRAIN_UPDATE_SESSION:-0}"
_budget=16; [ "$_session" = 1 ] && _budget=8   # 8s manual · 4s automated session check
# The remote wins on tags: a same-named tag from the other lineage (LAN vs
# public) is overwritten, not refused in silence (see scripts/lib/lineage.sh).
g fetch --quiet --force --prune-tags --tags "$remote" 2>/dev/null &
_fpid=$!
_i=0
while [ "$_i" -lt "$_budget" ]; do
  kill -0 "$_fpid" 2>/dev/null || break
  sleep 0.5
  _i=$((_i + 1))
done
if kill -0 "$_fpid" 2>/dev/null; then
  ...
  [ "$_session" = 1 ] && exit 0
  die "fetch from '$remote' timed out (unreachable?) — nothing changed"
fi
if ! wait "$_fpid"; then
  [ "$_session" = 1 ] && exit 0
...
if [ "$target" = "$cur_sha" ] || g merge-base --is-ancestor "$target" HEAD 2>/dev/null; then
  # Evidence a human can read: the version, not just a sha.
  desc="$(g describe --tags --match 'v*' 2>/dev/null || true)"
  ok "up to date — '$channel_name' is at $short_t${desc:+ ($desc)}, nothing changed"
  exit 0
fi
...
# 5. Re-wire BEFORE the gate: the new code's checks may require the new
# wiring (e.g. a renamed skill whose links only exist after setup-skills).
# Idempotent + best-effort; on rollback the old code re-wires itself back.
rewire() {
  # One wiring primitive: `brain wire` (brain.sh). Best-effort here — wire's own
  # warnings carry the run-hints. A rollback can land on a checkout that predates
  # the CLI, so probe the help text and fall back to the direct scripts.
  if bash "$REPO/scripts/brain.sh" help 2>/dev/null | grep -q ' wire '; then
    bash "$REPO/scripts/brain.sh" wire --quiet </dev/null || true
    return 0
  fi
  if [ -f "$REPO/scripts/setup/setup-skills.sh" ]; then
    bash "$REPO/scripts/setup/setup-skills.sh" >/dev/null 2>&1 </dev/null || \
      warn "skill re-wire skipped/failed — run: bash $REPO/scripts/setup/setup-skills.sh"
  fi
  if [ -f "$REPO/scripts/configure-pi.sh" ] && { [ -d "${AGENTBRAIN_HOME:-$HOME}/.pi/agent" ] || command -v pi >/dev/null 2>&1; }; then
    bash "$REPO/scripts/configure-pi.sh" >/dev/null 2>&1 </dev/null || \
...
if eval "$DOCTOR_CMD" >"$doctor_log" 2>&1; then
  ...
  rewire   # restore the OLD code's wiring so the machine stays consistent
