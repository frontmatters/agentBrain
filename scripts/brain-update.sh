...
if [ "$SESSION" -eq 1 ]; then
  ...
  m="$(python3 -c "import json,os;p='$CFG';print(json.load(open(p)).get('auto_update','notify') if os.path.exists(p) else 'off')" 2>/dev/null || echo off)"
  ...
  case "$m" in
    ...
    ask)
      ...
      ;;
    auto)   bash "$0" ${REPO:+--repo "$REPO"} >/dev/null 2>&1 || true ;;  # update or stay put
  esac
...
if eval "$DOCTOR_CMD" >/tmp/brain-update-doctor.log 2>&1; then
  ...
  tail -5 /tmp/brain-update-doctor.log >&2 || true
  if [ "$ff_possible" -eq 1 ]; then
