...
Usage:
  ...
  ./scripts/uninstall.sh --purge          Also remove vault/addons/<id>/ config dirs.
...
confirm() {
	...
	if [ -t 0 ]; then
		ab_prompt_confirm --default no "$1"
		return
	fi
	return 1
}
...
for skills_dir in "$AGENT_HOME/.claude/skills" "$AGENT_HOME/.copilot/skills"; do
	...
	for link in "$skills_dir"/*; do
		[[ -L "$link" ]] || continue
		tgt="$(readlink "$link")"
		if [[ "$tgt" == *"/system/skills/"* || "$tgt" == *"/local/skills/"* || "$tgt" == *"/vault/skills/"* ]]; then
			rm -f "$link"
...
if [ -d "${VAULT}/vault/addons" ]; then
	for _marker in "${VAULT}/vault/addons"/*/enabled; do
		[ -f "$_marker" ] || continue
...
if [ "${#ENABLED_ADDONS[@]}" -gt 0 ]; then
	...
	for _addon_id in "${ENABLED_ADDONS[@]}"; do
		...
		rm -f "${VAULT}/vault/addons/${_addon_id}/enabled"
		log_removed "Addon enabled marker: ${_addon_id}"
		if [ "$PURGE_ADDON_CONFIGS" = true ] && [ -d "${VAULT}/vault/addons/${_addon_id}" ]; then
			rm -rf "${VAULT}/vault/addons/${_addon_id}"
			log_removed "Addon config dir: vault/addons/${_addon_id}/ (--purge)"
		fi
	done
else
...
if [ "$(uname)" = "Darwin" ]; then
	...
	# The two LAN jobs of a factory host (git daemon, lan-install), rendered by
	# setup-launchd-lan.sh. Same sandbox rule as the loop job above.
	for _lan_label in dev.agentbrain.git-daemon dev.agentbrain.lan-install; do
		_lan_plist="${AGENT_HOME}/Library/LaunchAgents/${_lan_label}.plist"
		if [ -f "$_lan_plist" ] || { [ "$AGENT_HOME" = "$HOME" ] && launchctl list 2>/dev/null | grep -q "$_lan_label"; }; then
			launchctl bootout "gui/$(id -u)/${_lan_label}" 2>/dev/null || true
			rm -f "$_lan_plist"
			log_removed "launchd job: ${_lan_label}"
		else
			log_skip "launchd job: ${_lan_label}"
		fi
	done
	...
	for _addon_plist in "${AGENT_HOME}/Library/LaunchAgents"/local.agentbrain.*.plist; do
		...
		if ! bash "${VAULT}/scripts/setup/setup-addon-launchd.sh" uninstall "$_addon_id" 2>/dev/null; then
...
echo "    - Update config:     vault/update/ (channel + auto_update settings)"
echo ""

# ── 5b. The vault alias symlink ─────────────

# An older install may still carry local/ as an alias of vault/. Only ever
# remove OUR symlink: a real directory named local is somebody's data. The
# vault/ link itself stays: it is the way to the knowledge, which uninstall
# never touches.
LOCAL_ALIAS="${VAULT}/local"
if [ -L "$LOCAL_ALIAS" ] && [ "$(readlink "$LOCAL_ALIAS")" = "vault" ]; then
	rm -f "$LOCAL_ALIAS"
	echo -e "${GREEN}Removed${NC} local/ alias symlink (vault/ untouched)"
elif [ -e "$LOCAL_ALIAS" ]; then
	echo "  local/ exists but is not our alias symlink — left alone."
fi
...
echo ""
echo "Done. Re-run 'bash scripts/setup/setup.sh' to reinstall if needed."
