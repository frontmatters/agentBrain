...
#
# Bumped by tools/framework-bump-version.sh (factory) — see that script for write-side enforcement.
...
if [ ! -f "$VERSION_FILE" ]; then
	...
	echo "  Fix: tools/framework-bump-version.sh (factory) patch  (or create the file with an initial X.Y.Z)" >&2
	exit 1
fi
...
echo "  Fix:      tools/framework-bump-version.sh (factory) --show     (inspect current)" >&2
echo "            tools/framework-bump-version.sh (factory) patch      (clean bump)" >&2
exit 1
