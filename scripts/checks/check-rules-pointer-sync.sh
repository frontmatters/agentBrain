...
#
# Usage: bash scripts/checks/check-rules-pointer-sync.sh
...
EXPECTED_CLAUSES=(
	...
	"system/skills.md"
	"vault/sessions/startup-context.md"   # added 2026-05-24 (Phase 3 surface)
...
# The pointer's reading list must also appear in the canonical hot startup set
# (system/rules.md, Self-Learning Protocol Step 1) — the two are documented as
# identical, so a file added to one but not the other is drift.
RULES_MD="$ROOT_DIR/system/rules.md"
READING_LIST_CLAUSES=(
	"vault/learnings/patterns.md"
	"vault/learnings/troubleshooting.md"
	"system/agent-config/shared.md"
	"system/skills.md"
	"vault/sessions/startup-context.md"
)
for clause in "${READING_LIST_CLAUSES[@]}"; do
	if grep -qF "$clause" "$RULES_MD"; then
		echo "  ✓ rules.md Step-1 hot set contains: '$clause'"
		PASS=$((PASS+1))
	else
		echo "  ✗ rules.md Step-1 hot set MISSING pointer clause: '$clause'" >&2
		FAIL=$((FAIL+1))
	fi
done
