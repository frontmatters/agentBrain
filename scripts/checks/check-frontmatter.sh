...
is_exempt_template() {
	...
	# Skill-owned reference docs (e.g. system/skills/component-motion/references/animejs.md) are
	# vendored documentation carrying their own name/description header (often behind a PROVENANCE
	# comment), not vault notes — exempt like templates. README.md inside references/ IS a real note.
	if [[ "$file" == system/skills/*/references/* ]] && [[ "$file" == *.md ]] && [[ "$(basename "$file")" != "README.md" ]]; then
		return 0
	fi
	return 1
}
...
_yaml_available=""
yaml_available() {
	# PyYAML is OPTIONAL (absent on a fresh machine): the strict-YAML gate then
	# soft-skips VISIBLY — never a silent set -e death via a failing $() assignment.
	if [ -z "$_yaml_available" ]; then
		if command -v python3 >/dev/null 2>&1 && python3 -c 'import yaml' >/dev/null 2>&1; then
			_yaml_available=yes
		else
			_yaml_available=no
			echo "Note: PyYAML not available — strict YAML frontmatter validation skipped (runs fully on machines with PyYAML)." >&2
		fi
	fi
	[ "$_yaml_available" = yes ]
}
...
# Framework files only. In a git checkout that is what is tracked; a stray
# root file left by an earlier lineage (an old RELEASE_NOTES.md) is not the
# framework's to validate. Without git (a release payload) fall back to find.
# A function, not an inline block: bash 3.2 (macOS /bin/bash) cannot parse a
# multi-line if inside <( ... ) and died here with "no closing )" (2026-09-07).
framework_md_files() {
	if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
		git ls-files -z -- '*.md' | tr '\0' '\n' | sed 's|^|./|'
	else
		# No git (a release payload): the framework's own trees, plus the root
		# files the framework ships. A stray root file is not the framework's.
		find ./system ./docs ./templates ./scripts ./.github -type f -name '*.md' 2>/dev/null
		for f in README.md CHANGELOG.md CLAUDE.md SECURITY.md NOTICE.md; do [ -f "./$f" ] && echo "./$f"; done
	fi
}
...
while IFS= read -r file; do
	...
	# local/ is the vault's old name; on an updated checkout it can be a real
	# directory left behind by an earlier lineage. Vault material either way.
	[[ "$file" == ./.git/* || "$file" == ./vault/* || "$file" == ./local/* || "$file" == *node_modules/* ]] && continue
	...
	# Impeccable writes its product record and critique snapshots here in its own
	# schema — tool-managed docs, same class as the superpowers artifacts above.
	[[ "$file" == ./.impeccable/* ]] && continue
	file="${file#./}"
