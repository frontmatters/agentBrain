...
if [[ -d vault/sessions/archive ]]; then
	...
	done < <(find vault/sessions/archive -type f -name '*.md' | sort)
fi

if [[ -f vault/sessions/session-journal.md ]]; then
	previous="$(grep -E '^previous:' vault/sessions/session-journal.md | head -1 | sed 's/^previous:[[:space:]]*//')"
