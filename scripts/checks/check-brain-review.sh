...
# Usage:
#   bash scripts/checks/check-brain-review.sh           # check both public and local
#   bash scripts/checks/check-brain-review.sh --public   # check public layer only
#   bash scripts/checks/check-brain-review.sh --local    # check local layer only
...
# sanitization/deletion — check-vault-private.sh already excludes it for the
...
# 'extracted' holds machine-extracted learnings with their own content-hash schema
# (contentHash/extractedAt/source, no type field) — the note-schema check only
# produces noise on them, and the id-validator already exempts them by path.
# 'claudebrain-sessions' is a legacy raw session archive, not curated notes.
# 'skills' / 'addons' are package trees (bin/, references/, workspaces, benchmark
# fixtures), not curated knowledge notes — they have their own validators
# (check-skills-index, check-skill-links, check-addons, check-frontmatter). Only
# SKILL.md/manifest.md were exempt before, leaving package docs/workspaces (e.g.
# skills/hallmark, addons/core-bench train-vault fixtures) to produce pure noise.
# 'graphify-out' / 'graphify-out-stray-backup' are tool-generated graph reports —
# already excluded by check-vault-content.sh and local/.searchignore; brain-review
# was the only check still scanning them.
# 'repo-distill' is repo-distill/scanman tool output (handovers, empirical runs).
# 'uxray' is generated UI-audit report output under analyses/, not a curated note.
# 'spaces' holds sealed client spaces with their own hygiene + remote — reviewing
# them in the personal vault is a category error (they are not personal notes).
skip_segments = {'youtube-digest', 'daily-notes', 'sessions', 'setup-history', 'templates', 'quarantine', 'archive', 'extracted', 'claudebrain-sessions', 'skills', 'addons', 'graphify-out', 'graphify-out-stray-backup', 'repo-distill', 'uxray', 'spaces'}
...
if scope in ('all', 'public'):
    ...
    public_dirs = ['system', 'templates', 'docs']
...
if scope in ('all', 'public'):
    for d in ['system', 'templates', 'docs']:
        if os.path.isdir(d):
