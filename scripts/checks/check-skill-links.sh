...
is_brain_skill_link() {
	...
	[[ "$target" == *"/system/skills/"* || "$target" == *"/local/skills/"* || "$target" == *"/vault/skills/"* ]]
}
...
for _entry in "${AGENTS[@]}"; do
	...
	if [ -n "$_det_cmd" ] && command -v "$_det_cmd" &>/dev/null && "$_det_cmd" --version </dev/null >/dev/null 2>&1; then
		...
		# binary alone must not make a sandbox demand Pi links that were never configured.
		if [ "$_label" != "Pi" ] || [ -d "$_det_dir" ]; then _detected=true; fi
	fi
	if "$_detected"; then
		detected=$((detected + 1))
