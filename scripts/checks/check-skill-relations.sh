...
#
# Usage: bash scripts/checks/check-skill-relations.sh

set -uo pipefail

resolve_brain_dir() {
    local candidate
    for candidate in "${AGENTBRAIN_DIR:-}" "${AGENTBRAIN_HOME:-$HOME}/agentBrain" "$HOME/agentBrain" "$HOME/Developer/agentBrain"; do
        [ -n "$candidate" ] || continue
        if [ -d "$candidate" ]; then
            (cd "$candidate" && pwd -P) && return 0
        fi
    done
    echo "check-skill-relations: cannot resolve the active agentBrain root (set AGENTBRAIN_DIR or AGENTBRAIN_HOME)" >&2
    exit 1
}
