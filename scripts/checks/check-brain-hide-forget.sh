...
mkdir -p "$TMPD/vault" && ln -sfn vault "$TMPD/local"
trap 'rm -rf "$TMPD"' EXIT

mkdir -p "$TMPD/vault/learnings" "$TMPD/system/lib"
cp "$ROOT_DIR/system/lib/visibility.sh" "$TMPD/system/lib/"

for slug in keep hide forget; do
    cat > "$TMPD/vault/learnings/$slug.md" <<EOF
---
date: 2026-06-03
type: learning
tags: [test]
...
if grep -q "^hidden: true$" "$TMPD/vault/learnings/hide.md"; then
...
if [ ! -f "$TMPD/vault/learnings/forget.md" ]; then
    ok "brain-forget removes file"
else
    bad "brain-forget left file"
fi
trashed=$(find "$TMPD/vault/.trash/forget" -name "forget.md" | head -1)
if [ -n "$trashed" ]; then
...
flags=$(grep -c "^hidden:" "$TMPD/vault/learnings/hide.md" || true)
if [ "$flags" -eq 0 ]; then
...
TS=""
for d in "$TMPD/vault/.trash/forget"/*/; do
    [ -d "$d" ] || continue
    TS=$(basename "$d")
    break
done
...
# keep.md untouched
if [ -f "$TMPD/vault/learnings/keep.md" ]; then
