...
# shellcheck disable=SC2329,SC2317  # invoked indirectly via trap below
restore_flip() {
...
if bash "$DEV_DIR/scripts/checks/doctor.sh" >/dev/null 2>&1; then
	record "dev doctor passes" pass
else
	record "dev doctor passes" fail "non-zero exit"
fi
if bash "$LIVE_DIR/scripts/checks/doctor.sh" >/dev/null 2>&1; then
