...
for check in "${all_checks[@]}"; do
	...
	if [ "$SUMMARY" = true ]; then
		...
		if [ "$rc" -eq 0 ]; then
			...
			# `|| true` on both: a check that fails without any keyword in its output
			# (check-onboarding lists template files, test-report-stale prints its own
			# tally) leaves the first grep empty, the second grep then exits 1 on empty
			# input, and under pipefail the assignment fails and set -e ends the doctor
			# in silence, exactly where the reason should have been. ugrep on the
			# maintainer machine exits 0 there, BSD and GNU grep do not (2026-09-07).
			reason="$(echo "$output" | grep -A3 -E '(✗|FAIL|failed|Error|FATAL|touched the checkout|was:|now:)' | grep -vE '^--$' | tail -12 || true)"
			[ -n "$reason" ] || reason="$(echo "$output" | grep -vE '^[[:space:]]*$' | tail -8 || true)"
			printf '%s\n' "$reason" | sed 's/^/   │ /'
			printf '   └ open: %s\n' "$check"
		fi
	else
