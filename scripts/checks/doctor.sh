...
for check in "${all_checks[@]}"; do
	...
	if output="$(env -u VAULT -u AGENTBRAIN_DIR -u AGENTBRAIN_VAULT_DIR -u AGENTBRAIN_LOCAL_DIR -u BRAIN_DIR -u BRAIN_ALIAS bash -c "$check" 2>&1)"; then rc=0; else rc=1; fi
	touched=""
	...
	if [ "$SUMMARY" = true ]; then
		...
		if [ "$rc" -eq 0 ]; then
			...
			reason="$(echo "$output" | grep -E '(✗|FAIL|failed|Error|FATAL|touched the checkout|was:|now:)' | tail -8)"
