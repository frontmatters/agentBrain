	...
	while IFS= read -r ref; do
		if [ ! -e "$ref" ]; then
			case "$ref" in
				...
				local | local/* | vault | vault/*) continue ;;
				...
				scripts/release/publish-addon.sh | scripts/release/publish-agentbrain-github.sh | scripts/release/mirror-registry-github.sh | scripts/release/bump-version.sh | scripts/release.sh | scripts/release/publish-gitea-release.sh | scripts/sync/dev-sync-status.sh | scripts/release/deploy-dev-to-live.sh | scripts/release-check.sh | scripts/checks/validate-install.sh) continue ;;
