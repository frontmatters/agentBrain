...
scripts=(
	...
	scripts/setup/setup-agent-integrations.sh
	scripts/configure-pi.sh
	scripts/installer/bootstrap/macos.sh
	scripts/flow/ensure-daily-note.sh
	scripts/offboard.sh
	scripts/import-offboard.sh
	scripts/uninstall.sh
	scripts/sync/move-agentbrain.sh
)
...
grep -q -- '--move-to' scripts/setup/setup.sh || fail 'setup.sh missing --move-to relocation entrypoint'
grep -q 'move-agentbrain.sh' scripts/setup/setup.sh || fail 'setup.sh does not delegate relocation to move-agentbrain.sh'
...
grep -q 'Backup' scripts/sync/move-agentbrain.sh || fail 'move-agentbrain.sh missing backup step'
grep -q 'doctor.sh' scripts/sync/move-agentbrain.sh || fail 'move-agentbrain.sh does not validate with doctor after move'
grep -q 'Relinked' scripts/sync/move-agentbrain.sh || fail 'move-agentbrain.sh missing Pi symlink relink behavior'

grep -q 'vault/daily-notes' scripts/flow/ensure-daily-note.sh || fail 'ensure-daily-note.sh missing daily note target'
grep -q 'templates/daily.md' scripts/flow/ensure-daily-note.sh || fail 'ensure-daily-note.sh missing template usage'
