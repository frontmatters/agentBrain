...
while IFS= read -r pj; do
	[[ -z "$pj" ]] && continue
	# `|| true`: a package.json without a "license" field makes grep exit 1,
	# which under `set -e` would kill the check silently. The logic below only
	# flags a WRONG license, so a missing field must resolve to empty, not death.
	spdx="$(grep -m1 '"license"' "$pj" | sed -E 's/.*"license"[[:space:]]*:[[:space:]]*"([^"]*)".*/\1/' || true)"
