...
for file in "${client_configs[@]}"; do
	require_pattern "$file" 'vault/preferences/organization/' "organization preference scope in $file"
	require_pattern "$file" 'vault/preferences/team/' "team preference scope in $file"
	require_pattern "$file" 'vault/preferences/personal/' "personal preference scope in $file"
done
...
for file in system/agent-config/claude.md system/agent-config/copilot.md system/agent-config/windsurf.md system/agent-config/gemini.md; do
	require_pattern "$file" 'vault/daily-notes' "daily note pointer in $file"
done
...
require_pattern scripts/agentbrain-pointer.sh 'vault/preferences/organization/' 'pointer block organization scope'
require_pattern scripts/agentbrain-pointer.sh 'vault/preferences/team/' 'pointer block team scope'
require_pattern scripts/agentbrain-pointer.sh 'vault/preferences/personal/' 'pointer block personal scope'
require_pattern scripts/agentbrain-pointer.sh 'system/agent-config/' 'pointer block references per-agent config'
require_pattern scripts/setup/setup-claude-code.sh 'agentbrain_pointer_block' 'setup-claude-code uses the shared pointer block'
require_pattern scripts/setup/setup-gemini-cli.sh 'GEMINI\.md' 'setup-gemini-cli writes GEMINI.md'
require_pattern scripts/setup/setup-gemini-cli.sh 'gemini.md' 'setup-gemini-cli passes its agent-config'
...
if grep -R 'Preferences:.*vault/preferences/`\|Preferences:.*vault/preferences/$' system/agent-config scripts/setup/setup.sh .github/copilot-instructions.md 2>/dev/null; then
