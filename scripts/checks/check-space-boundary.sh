...
#       The personal sync (scripts/sync/sync-vault.sh) runs `git add -A`
...
# belongs in the shared vault. So `confidential: false` is not a state a space
# can be in, and a readable `display:` name (which would travel wherever the
# slug does) has no place on one. A note that merely wants a readable name is
# not a space; move it to the shared vault. Measured once: a public-project
# review sat in a "non-confidential space" with sync: none and no backup.
for _idx in "$LOCAL_DIR"/spaces/*/index.md; do
	[ -f "$_idx" ] || continue
	if grep -qm1 '^confidential:[[:space:]]*false' "$_idx"; then
		err "Space '$(basename "$(dirname "$_idx")")' says confidential: false. A space is confidential by definition; reusable knowledge goes to the shared vault (rules.md)."
		fail=1
	fi
	if grep -qm1 '^display:' "$_idx"; then
		err "Space '$(basename "$(dirname "$_idx")")' carries a display name. A readable name would travel wherever the slug does; a space has none."
		fail=1
	fi
done
