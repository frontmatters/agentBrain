...
#       The personal sync (scripts/sync/sync-agentbrain-local.sh) runs `git add -A`
...
scan_public() {
	local flags="$1" lit="$2"
	if root_is_git; then
		git grep --untracked -nI"$flags" -F -e "$lit" -- . ':(exclude)vault/**' 2>/dev/null || true
	else
...
# ── (0) A readable name may only exist where there is nothing to hide ────────
# `display:` pairs a human name with the opaque slug so a listing reads as
# "ecc-review (sp-e5e12dee)" instead of a hex string nobody recognises. That is
# a real usability gain and a real leak risk, and the confidentiality flag is
# what separates them: a confidential space carries no display name, so the
# pairing can never spell out an owner. Enforced here rather than trusted,
# because the whole point of the slug is that trusting people to remember is
# what fails.
for _idx in "$LOCAL_DIR"/spaces/*/index.md; do
	[ -f "$_idx" ] || continue
	grep -qm1 '^display:' "$_idx" || continue
	if grep -qm1 '^confidential:[[:space:]]*true' "$_idx"; then
		err "Space '$(basename "$(dirname "$_idx")")' is confidential and carries a display name."
		err "A readable name would travel wherever the slug does. Remove display:, or set confidential: false if there is genuinely nothing to hide."
		fail=1
	fi
done
...
if vault_is_git; then
	# Staged paths are repo-toplevel-relative. On a dedicated vault repo (or the
	# symlinked shared vault) the prefix is ""; on a consumer layout where local/
	# is a plain dir inside the framework checkout, git resolves to the enclosing
	# repo and spaces/ paths are staged as local/spaces/ — a bare ^spaces/ grep
	# would silently miss the breach there.
	vault_prefix="$(git -C "$LOCAL_DIR" rev-parse --show-prefix 2>/dev/null || true)"
	# One path under spaces/ must be tracked: the fail-closed .gitignore that
	# keeps everything else out. It carries no space content, and a layer that
	# cannot be committed does not survive a fresh clone, which is the machine
	# where the seal matters most. Everything else under spaces/ is still a breach.
	breach="$(git -C "$LOCAL_DIR" diff --cached --name-only 2>/dev/null \
		| grep -E "^${vault_prefix}spaces/" \
		| grep -vE "^${vault_prefix}spaces/\.gitignore$" || true)"
	if [ -n "$breach" ]; then
