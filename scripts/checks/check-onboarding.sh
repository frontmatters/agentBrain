...
set -euo pipefail
# The checkout this check belongs to, by its own location. setup.sh exports
# VAULT for its own children; a check must not inherit it (a fixture test that
# ran under setup then checked the real vault instead of its fixture).
VAULT="$(cd -P "$(dirname "$(realpath "$0")")/../.." && pwd -P)"
PREFS="$VAULT/vault/preferences/personal"
...
if [ ! -d "$PREFS" ] || [ "${#incomplete[@]}" -gt 0 ]; then
  ...
  if [ ! -d "$PREFS" ]; then
    echo "  vault/preferences/personal/ missing"
  else
