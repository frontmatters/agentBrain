...
#   bash scripts/checks/check-local-content.sh                      # whole local/
#   bash scripts/checks/check-local-content.sh local/flux local/learnings   # scoped
#   bash scripts/checks/check-local-content.sh --strict             # hard-fail on everything
#   bash scripts/checks/check-local-content.sh --json               # structured output for the loop
#
...
if [ ! -d local ]; then
	echo "check-local-content: no vault/ — skip (PASS)"
	exit 0
fi
...
def load_exempt_patterns() -> list:
    ...
    if not EXEMPT_FILE.is_file():
        print(f"check-local-content: missing {EXEMPT_FILE}", file=sys.stderr)
        raise SystemExit(1)
    pats = []
    for line in EXEMPT_FILE.read_text(encoding="utf-8").splitlines():
        ...
        if line and not line.startswith("#"):
            pats.append(line)
    if not pats:
        print(f"check-local-content: {EXEMPT_FILE} has no patterns", file=sys.stderr)
        raise SystemExit(1)
    return pats
