...
if [[ ! -f "${VAULT}/brain.json" ]]; then
	echo "ERROR: brain.json not found. Run scripts/setup/setup.sh first." >&2
	exit 1
fi
...
PREFS_DST="${VAULT}/vault/preferences"
if [[ -d "$PREFS_SRC" ]]; then
...
DAILY_DST="${VAULT}/vault/daily-notes"
if [[ -d "$DAILY_SRC" ]]; then
	mkdir -p "$DAILY_DST"
...
LEARN_DST="${VAULT}/vault/learnings"
if [[ -d "$LEARN_SRC" ]]; then
	mkdir -p "$LEARN_DST"
...
echo ""
echo "  Next steps:"
echo "    1. Review imported personal preferences: ls vault/preferences/personal/"
