...
if [[ $# -lt 1 ]]; then
	echo "Usage: scripts/sync/move-agentbrain.sh <NEW_ABSOLUTE_PATH>"
	echo ""
	echo "Example: scripts/sync/move-agentbrain.sh /Volumes/SSD2/agentBrain"
	exit 1
fi
...
echo "  To:   ${NEW_VAULT}"
echo ""
ab_prompt_confirm --default no "Proceed?"
echo
[[ $REPLY =~ ^[Yy]$ ]] || {
	echo "Aborted."
	exit 0
...
echo ""
echo "Running doctor..."
VAULT="$NEW_VAULT" bash "${NEW_VAULT}/scripts/checks/doctor.sh" --summary
