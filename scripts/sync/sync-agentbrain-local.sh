...
LOCAL_DIR="$VAULT_DIR"
CHECK_SCRIPT="${AGENTBRAIN_LOCAL_CHECK_SCRIPT:-$ROOT_DIR/scripts/checks/check-agentbrain-local.sh}"

# Pre-flight drift report (read-only): uncommitted work, merge debt and missing
# bumps, for the vault and known sibling repos. Report-only: it never blocks a
# sync, it makes drift visible before anything moves.
echo "==> Pre-flight change report"
bash "$ROOT_DIR/scripts/changes.sh" || true
echo ""
...
# self-heal so re-clones / new machines get them without a manual step.
#
# Into the directory core.hooksPath names, when set: a hook dropped into
# .git/hooks is inert on a repo that points hooksPath elsewhere, which is how
# the maintainer's vault ran a private four-layer hook for months while the
# framework's one-layer copy sat unused beside it.
HOOK_DIR="$(git -C "$LOCAL_DIR" config core.hooksPath 2>/dev/null || true)"
HOOK_DIR="${HOOK_DIR:+$LOCAL_DIR/$HOOK_DIR}"; HOOK_DIR="${HOOK_DIR:-$LOCAL_DIR/.git/hooks}"
mkdir -p "$HOOK_DIR"
for hook in pre-commit post-commit; do
	HOOK_SRC="$ROOT_DIR/scripts/hooks/vault-$hook.sh"
	HOOK_DST="$HOOK_DIR/$hook"
	if [[ -f "$HOOK_SRC" ]] && ! cmp -s "$HOOK_SRC" "$HOOK_DST" 2>/dev/null; then
		install -m 0755 "$HOOK_SRC" "$HOOK_DST" 2>/dev/null && log "Installed vault $hook gate ($HOOK_DST)"
	fi
done
...
if ! bash "$ROOT_DIR/scripts/hooks/validate-staged-note-ids.sh"; then
