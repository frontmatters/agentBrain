...
#
# Usage:
#   bash scripts/sync/sync-space.sh <slug> [commit-message]
#
#   sync: none (or empty) -> local-only: versioned and committed, but no
#                            remote and therefore no backup. Exit 0.
...
if [ -z "$SYNC" ] || [ "$SYNC" = "none" ] || [ "$SYNC" = "null" ]; then
	# Local-only still means versioned. Backup needs a remote; history does not,
	# and tying the two together left a space unversioned until somebody
	# remembered to seal it. Committing here also makes the guarantee hold for a
	# space created by hand, not only for one that came through new-space.sh.
	# -b main to match what the remote path creates below.
	if [ ! -e "$SPACE_DIR/.git" ]; then
		git -C "$SPACE_DIR" init -q -b main . 2>/dev/null \
			&& log "space '$SLUG': versioned locally (no remote)"
	fi
	if [ -e "$SPACE_DIR/.git" ]; then
		git -C "$SPACE_DIR" add -A 2>/dev/null || true
		if ! git -C "$SPACE_DIR" diff --cached --quiet 2>/dev/null; then
			git -C "$SPACE_DIR" commit -q -m "$MESSAGE" 2>/dev/null \
				&& log "space '$SLUG': committed changes"
		fi
	fi
	echo "space '$SLUG': local-only, versioned but NOT backed up (sync: none)"
	exit 0
fi
