...
lines = [
    ...
    "",
    "> AUTO-GENERATED door `scripts/reports/render-findings-backlog.sh` (loop-tick).",
...
for sev in SEV_ORDER + sorted(set(by_sev) - set(SEV_ORDER)):
    ...
    if len(items) > CAP_PER_SEVERITY:
        lines.append(f"- … nog {len(items) - CAP_PER_SEVERITY} {sev}-findings: zie `vault/findings/*.json`")
    lines.append("")

lines.append("## Werkinstructie")
lines.append("")
lines.append("Pak findings van boven naar beneden (error eerst). Na het fixen:")
lines.append("`bash scripts/flow/loop-tick.sh` draaien (of de nachtelijke tick afwachten)")
