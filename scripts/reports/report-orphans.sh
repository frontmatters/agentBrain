...
# report-orphans.sh — report NOTES that hang loose in the knowledge graph.
#
...
#
# Usage:
#   bash scripts/reports/report-orphans.sh                 # count + per-folder breakdown
...
for arg in "$@"; do
	case "$arg" in
		...
		--*) echo "report-orphans: unknown flag $arg" >&2; exit 2 ;;
		*) SCOPE+=("$arg") ;;
	esac
done
...
if (JSON_OUT) {
	...
	console.log(`report-orphans: ${rels.length} orphan(s) — ${label}`);
