...
#     "canonical":    { "<slug>": "/abs/expanded/canonical-code-root", ... }
#   }
#
...
# Resolve through symlinks: this script is also reachable as scripts/build-space-map.sh,
# and BASH_SOURCE then points at the link, not at this file — which would put ROOT one
# level too high and write the map outside the brain.
SELF="${BASH_SOURCE[0]}"
while [ -L "$SELF" ]; do
	LINK="$(readlink "$SELF")"
	case "$LINK" in
...
by_code_root, by_alias, canonical_map = {}, {}, {}
...
for name in sorted(os.listdir(spaces_dir)):
    ...
    front = parts[1]
    slug, aliases, code_roots, canonical = name, [], [], ''
    for line in front.splitlines():
        ...
        elif re.match(r'^canonical:', line):
    ...
    if canonical:
        canonical_map[slug] = expand(canonical)

data = {"by-alias": by_alias, "by-code-root": by_code_root, "canonical": canonical_map}
with open(out, 'w', encoding='utf-8') as fh:
    fh.write(json.dumps(data, indent=2, sort_keys=True) + "\n")

print(f"build-space-map: {len(by_code_root)} code-root(s), {len(by_alias)} alias(es), {len(canonical_map)} canonical → {out}")
PY
