...
# Where <detector> is the script base name (no .sh), e.g. "check-vault-content".
...
if [ $# -ne 1 ]; then
	...
	echo "  e.g. $0 check-vault-content" >&2
	exit 2
fi
...
# stranded temp when the detector or the merge aborts.
TMP_FILE="$(mktemp "${FINDINGS_FILE}.tmp.XXXXXX")"
