...
# The release artifact ships only the ESSENTIAL addons; the rest are distributed
# via the addon registry. Shipped indexes (system/addons/README.md, skill docs)
# legitimately link to those registry addons — their ids from clients.md count
# as valid stems so a pruned artifact doesn't flag them as broken.
pruned = Path('system/addons/REGISTRY-ADDONS.txt')
if pruned.exists():
    for name in pruned.read_text(errors='ignore').split():
        public_stems.setdefault(name, []).append(pruned)
        all_stems.setdefault(name, []).append(pruned)
clients = Path('system/addons/clients.md')
if clients.exists():
    for line in clients.read_text(errors='ignore').splitlines():
        m = re.match(r'\|\s*([a-z0-9-]+)\s*\|', line)
        if m and m.group(1) not in ('addon', 'id'):
            public_stems.setdefault(m.group(1), []).append(clients)
            all_stems.setdefault(m.group(1), []).append(clients)

# Placeholder link-names used illustratively in system/templates/*.md — there is
# no real note named 'hostname'/'integration'/'device notes'; they teach the filler
# what to link to once the note exists (see also the `forward:` prefix mechanism).
example_targets = {'note-name', 'wiki-links', 'hostname', 'integration', 'device notes'}
...
def unresolved(files, stems):
    ...
    for path in files:
        try:
            ...
            # Broken symlink / unreadable file: a real repo defect — report it
            # visibly instead of dying with a traceback (fresh-machine honesty).
            out.append((path, '<unreadable — dangling symlink?>'))
            continue
