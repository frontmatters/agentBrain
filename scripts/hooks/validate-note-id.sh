...
#
# Usage:
#   bash scripts/hooks/validate-note-id.sh <file-path>
#
# Exit codes:
...
if [ -n "$CONTENT_FILE" ]; then
	...
	# local/ is the older name of the vault link; a note named that way lives under vault/ now.
	if [ ! -f "$FILE" ]; then
		case "$FILE" in
			local/*) [ -f "vault/${FILE#local/}" ] && FILE="vault/${FILE#local/}" ;;
			*/local/*) alt="${FILE/\/local\///vault/}"; [ -f "$alt" ] && FILE="$alt" ;;
		esac
	fi
...
REL="${REL#vault/}"
[[ "$REL" != "${ABS_FILE#"${BRAIN_ROOT}"/}" ]] && REL="local/${REL}"
...
# path-uuid5 check is a false positive — keep this list in sync with check-vault-content.sh.
case "$REL" in
