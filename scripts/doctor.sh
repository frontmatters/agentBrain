...
framework_checks=(
	...
	"bash scripts/check-explainers.sh"
	"bash scripts/check-architecture.sh"
	"bash scripts/check-learnings-structure.sh"
	"bash scripts/check-frontmatter.sh"
	"bash scripts/check-skill-relations.sh"
	"bash scripts/check-skills-index.sh"
	"bash scripts/check-ksc.sh"
	"bash scripts/check-release-published.sh"
	...
	"bash scripts/check-session-update-quiet.sh"
	...
	"bash scripts/check-prompt-cache-hygiene.sh"
...
local_checks=(
	...
	"bash scripts/test-check-prerequisites.sh"   # prerequisites preflight (clean-install)
	...
	"bash scripts/check-shorthand.sh"
	"bash scripts/check-brain-hide-forget.sh"
	"bash scripts/test-validate-note-id.sh"
	"bash scripts/test-new-note.sh"
	"bash scripts/test-new-note-space.sh"
	"bash scripts/test-queue.sh"
	"bash scripts/test-new-space.sh"
	"bash scripts/test-list-space.sh"
	"bash scripts/test-active-space.sh"
	"bash scripts/test-context.sh"
	"bash scripts/test-loop-tick.sh"
	"bash scripts/test-spaces-seal.sh"
	"bash scripts/test-spaces-extract.sh"
	"bash scripts/test-spaces-hygiene.sh"
	"bash scripts/test-sync-space.sh"
	"bash scripts/test-sync-agentbrain-local-cli.sh"
	"bash scripts/test-space-boundary.sh"
	"bash scripts/check-space-boundary.sh"
)
...
if [ "$CI_MODE" = true ]; then
	...
	for _i in "${!all_checks[@]}"; do
		if [ "${all_checks[_i]}" = "bash scripts/check-explainers.sh" ]; then
			all_checks[_i]="EXPLAINERS_DIR=/nonexistent bash scripts/check-explainers.sh"
		fi
	done
else
