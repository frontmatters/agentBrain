...
if [ -n "$BRAIN_BIN_DIR" ]; then
	BRAIN_BIN="$BRAIN_BIN_DIR/brain"
	# Never hijack an unrelated `brain` command: only (re)link when the target is
	...
	if [ -e "$BRAIN_BIN" ] || [ -L "$BRAIN_BIN" ]; then
		if [ -L "$BRAIN_BIN" ] && [ "$(basename "$(readlink "$BRAIN_BIN")")" = "brain.sh" ]; then
			...
			echo "  Symlink $VAULT/scripts/brain.sh onto your PATH yourself to use the 'brain' command."
		fi
	else
		ln -sfn "$VAULT/scripts/brain.sh" "$BRAIN_BIN"
	fi
...
# (scripts/onboard-wizard.sh). Optional — /onboard inside an agent later deepens
# via its skip-if-done contract (addons, org/team scopes, spaces, free prose).
if bash "${SCRIPTS}/check-onboarding.sh" >/dev/null 2>&1; then
	echo "Already personalized — /onboard can refine anytime."
elif [ -t 0 ]; then
	if confirm "Personalize your brain now? (5 min of quick questions, no AI needed)" "Run later: bash scripts/onboard-wizard.sh — or /onboard inside your agent." Y; then
		bash "${SCRIPTS}/onboard-wizard.sh" || echo "  Wizard skipped/failed — /onboard inside your agent covers the same ground."
	else
		echo "  Skipped. Later: bash scripts/onboard-wizard.sh — or /onboard inside your agent."
	fi
else
	echo "Non-interactive — skipped. Later: bash scripts/onboard-wizard.sh or /onboard."
fi
...
if [ "${AGENTBRAIN_SKIP_PI:-}" = 1 ]; then
	...
	if confirm "Configure Pi now?" "Pi is installed; this is the deep agentBrain integration." Y; then
		...
		if AGENTBRAIN_ASSUME_YES=1 bash "${SCRIPTS}/configure-pi.sh"; then
