...
out="$(bash "$BRAIN_DIR/scripts/checks/check-skill-relations.sh" 2>&1)"
echo "$out" | grep -qE "all relations valid|check-skill-relations: OK" && ok "check-skill-relations passes" || ko "check-skill-relations failed: $out"

out="$(bash "$BRAIN_DIR/scripts/checks/check-frontmatter.sh" 2>&1)"
