...
export ADDONS_STATE="$TEST_DIR/vault/addons"
...
mkdir -p "$AGENTBRAIN_HOME/.claude/skills" "$TEST_DIR/bin"
# The checker deliberately requires a real CLI/version probe, so provide a
# hermetic client stub rather than treating the config directory as installation evidence.
cat > "$TEST_DIR/bin/claude" <<'EOF'
#!/usr/bin/env bash
[ "${1:-}" = "--version" ] && printf 'claude-test 1.0\n'
EOF
chmod +x "$TEST_DIR/bin/claude"
# Keep host-installed Copilot from making this hermetic test detect a client
# whose isolated config directory does not exist.
cat > "$TEST_DIR/bin/copilot" <<'EOF'
#!/usr/bin/env bash
exit 1
EOF
...
CHK="bash scripts/checks/check-skill-links.sh"
ADDONS_ASSUME_YES=1 $ADDONS enable graphify >/dev/null
linked_check_out="$($CHK 2>&1 || true)"
assert "linked+enabled passes check"        "$(printf '%s' "$linked_check_out" | grep -q '0 failed' && echo 0 || echo 1)" "0"
[ "$(printf '%s' "$linked_check_out" | grep -q '0 failed' && echo 0 || echo 1)" = 0 ] || printf '%s\n' "$linked_check_out" >&2
...
# --- install: none is a documented value, not a command (goal, lottie-animator,
# web-interface-guidelines ship it). addons.sh ran `bash -c none` and the install
# failed with "none: command not found"; enabling by hand was the workaround.
mkdir -p "$ADDONS_REGISTRY/noinstall"
cat > "$ADDONS_REGISTRY/noinstall/manifest.md" <<'EOF'
---
id: noinstall
name: No Install
install: none
...
---
EOF
echo "# No Install" > "$ADDONS_REGISTRY/noinstall/README.md"
_ni_rc=0; _ni_out="$(ADDONS_ASSUME_YES=1 $ADDONS install noinstall 2>&1)" || _ni_rc=$?
assert "install: none installs without running a command (rc)" "$_ni_rc" "0"
assert "install: none never reaches the shell" "$(printf '%s' "$_ni_out" | grep -c 'command not found')" "0"
assert "install: none ends enabled" "$([ -f "$ADDONS_STATE/noinstall/enabled" ] && echo yes || echo no)" "yes"
rm -rf "$ADDONS_REGISTRY/noinstall" "$ADDONS_STATE/noinstall"
...
if [ "$(uname -s)" = "Darwin" ]; then
	LD="bash scripts/setup/setup-addon-launchd.sh"
...
assert "valid manifest passes" "$(bash scripts/checks/check-addons.sh >/dev/null 2>&1; echo $?)" "0"
...
assert "bad privacy fails" "$(bash scripts/checks/check-addons.sh >/dev/null 2>&1; echo $?)" "1"
cp "$TEST_DIR/good.md" "$ADDONS_REGISTRY/graphify/manifest.md"

# --- Task 8: per-add-on validation + test command ---
assert "check-addons single id ok" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh graphify >/dev/null 2>&1; echo $?)" "0"
# README is required by check-addons
rm -f "$ADDONS_REGISTRY/graphify/README.md"
assert "missing README fails" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh graphify >/dev/null 2>&1; echo $?)" "1"
echo "# Graphify" > "$ADDONS_REGISTRY/graphify/README.md"
assert "check-addons unknown id ok (no match, no error)" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh nope >/dev/null 2>&1; echo $?)" "0"
...
assert "missing install.sh fails" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile >/dev/null 2>&1; echo $?)" "1"
ref_err="$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile 2>&1 || true)"
...
assert "install.sh without uninstall.sh fails" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile >/dev/null 2>&1; echo $?)" "1"
sym_err="$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile 2>&1 || true)"
...
assert "install.sh present passes" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile >/dev/null 2>&1; echo $?)" "0"
...
assert "bare install.sh form passes" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile >/dev/null 2>&1; echo $?)" "0"
...
# Ref File
EOF
assert "missing entrypoint fails" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile >/dev/null 2>&1; echo $?)" "1"
ep_err="$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile 2>&1 || true)"
...
assert "present entrypoint passes" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile >/dev/null 2>&1; echo $?)" "0"
...
# Ref File
EOF
assert "external install not flagged" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile >/dev/null 2>&1; echo $?)" "0"
...
# Maintainer tooling lives in the factory beside this checkout, not in the
# framework. A consumer install has no factory; these tasks then skip.
FACTORY_TOOLS="${FACTORY_TOOLS:-$(cd -P "$ROOT_DIR/.." 2>/dev/null && pwd -P)/tools}"
# --- Task 18: package-addon ---
if [ -x "$FACTORY_TOOLS/addon-package.sh" ]; then
pkg_out="$(OUT_DIR="$TEST_DIR/out" bash "$FACTORY_TOOLS/addon-package.sh" localaddon \
...
: > "$ADDONS_STATE/localaddon/enabled"
OUT_DIR="$TEST_DIR/out2" bash "$FACTORY_TOOLS/addon-package.sh" localaddon --roots "$ADDONS_STATE:$ADDONS_REGISTRY" >/dev/null 2>&1
assert "enabled marker excluded" "$(unzip -l "$TEST_DIR/out2/addon-localaddon-v0.2.0.zip" | grep -c 'localaddon/enabled')" "0"
rm -f "$ADDONS_STATE/localaddon/enabled"
assert "package unknown fails"  "$(OUT_DIR="$TEST_DIR/out" bash "$FACTORY_TOOLS/addon-package.sh" nope --roots "$ADDONS_STATE:$ADDONS_REGISTRY" >/dev/null 2>&1; echo $?)" "1"
# dirty payload is refused by the privacy gate (FAKE_TOKEN from Task 17)
echo "$FAKE_TOKEN" > "$ADDONS_STATE/localaddon/secret.txt"
assert "dirty payload refused"  "$(OUT_DIR="$TEST_DIR/out3" bash "$FACTORY_TOOLS/addon-package.sh" localaddon --roots "$ADDONS_STATE:$ADDONS_REGISTRY" >/dev/null 2>&1; echo $?)" "1"
rm -f "$ADDONS_STATE/localaddon/secret.txt"
else
	echo "  (skip) Task 18: no factory tools at $FACTORY_TOOLS"
	# Task 19 tests registry-index.sh, a consumer script, against a packaged
	# addon. Without the factory's packager, build the same package by hand so
	# the consumer side is still tested on a consumer install.
	mkdir -p "$TEST_DIR/out" "$TEST_DIR/pkg/localaddon"
	cp "$ADDONS_STATE/localaddon/manifest.md" "$TEST_DIR/pkg/localaddon/"
	(cd "$TEST_DIR/pkg" && zip -qr "$TEST_DIR/out/addon-localaddon-v0.2.0.zip" localaddon)
	(cd "$TEST_DIR/out" && shasum -a 256 addon-localaddon-v0.2.0.zip > addon-localaddon-v0.2.0.zip.sha256)
fi
...
for s in addon-publish.sh addon-mirror-registry.sh; do
	...
	assert "no brace-eating default in $s" \
		"$(grep -cE '\$\{URL_TEMPLATE:-[^}]*\{(tag|file|id|version)\}' "$FACTORY_TOOLS/$s")" "0"
	# shellcheck disable=SC2016
	# The safe two-step assignment is present.
	assert "two-step URL default in $s" \
		"$(grep -c '\[ -n "\$URL_TEMPLATE" \] || URL_TEMPLATE=' "$FACTORY_TOOLS/$s")" "1"
done
...
CRP="bash $FACTORY_TOOLS/checks/check-release-published.sh"
if [ -x "$FACTORY_TOOLS/checks/check-release-published.sh" ]; then
...
assert "release-published warns with the cut command" "$(echo "$crp_warn" | grep -c 'publish-gitea')" "1"
else
	echo "  (skip) Task 26: no factory tools at $FACTORY_TOOLS"
fi
...
assert "missing onboard.run fails" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile >/dev/null 2>&1; echo $?)" "1"
ob_err="$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile 2>&1 || true)"
...
assert "present onboard.run passes" "$(ADDONS_CHECK_REGISTRY="$ADDONS_REGISTRY" bash scripts/checks/check-addons.sh reffile >/dev/null 2>&1; echo $?)" "0"
