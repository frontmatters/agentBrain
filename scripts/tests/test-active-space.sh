...
#   - DECOMMISSIONED write-role: the marker no longer routes note WRITES (those infer
#     context per-write); AGENTBRAIN_CONTEXT routes a write into a space
set -uo pipefail
ROOT_DIR="$(cd "$(dirname "$(realpath "${BASH_SOURCE[0]}")")/../.." && pwd)"
AS="$ROOT_DIR/scripts/flow/active-space.sh"
...
# A marker that already holds one of this test's own slugs is not a live
# session, it is what an earlier run left behind; backing THAT up and restoring
# it at the end is how __astest__ sat in local/.active-space for weeks, faithfully
# preserved by every later run. Stale test residue is discarded, not kept.
BACKUP=""
if [ -f "$MARKER" ]; then
	case "$(cat "$MARKER" 2>/dev/null)" in
	__astest*) rm -f "$MARKER" ;;
	*) BACKUP="$(mktemp)"; cp "$MARKER" "$BACKUP" ;;
	esac
fi
cleanup() {
	...
	# The test's last act is to check its own footprint.
	if [ -f "$MARKER" ] && grep -q '^__astest' "$MARKER" 2>/dev/null; then
		echo "FAIL: test left its own slug in $MARKER" >&2; exit 1
	fi
}
trap cleanup EXIT
...
# 5) DECOMMISSIONED write-role: with the marker set, new-note (no context env, from a
#    non-code-root cwd) must land in the MAIN vault — the marker no longer routes
#    WRITES, only recall.
bash "$AS" use "$SLUG" >/dev/null 2>&1
( cd "$ROOT_DIR" && env -u AGENTBRAIN_CONTEXT -u AGENTBRAIN_SPACE -u AGENTBRAIN_STRICT_CONTEXT \
	bash scripts/new-note.sh learning "$NOTE_REL" "Marker Ignored" >/dev/null 2>&1 )
[ -f "$NORMAL_NOTE" ] || { echo "FAIL: marker-set write did not land in the main vault"; exit 1; }
grep -q "^space:" "$NORMAL_NOTE" && { echo "FAIL: marker still routed a WRITE into the space (not decommissioned)"; exit 1; }
[ -f "$SPACE_NOTE" ] && { echo "FAIL: marker routed a write into the space"; exit 1; }
rm -f "$NORMAL_NOTE"
echo "PASS marker-does-not-route-writes"

# 6) new mechanism: AGENTBRAIN_CONTEXT routes a WRITE into the space, id-correct.
( cd "$ROOT_DIR" && AGENTBRAIN_CONTEXT="$SLUG" \
	bash scripts/new-note.sh learning "$NOTE_REL" "Context Env" >/dev/null 2>&1 )
[ -f "$SPACE_NOTE" ] || { echo "FAIL: AGENTBRAIN_CONTEXT write not created at $SPACE_NOTE"; exit 1; }
grep -q "^space: $SLUG" "$SPACE_NOTE" || { echo "FAIL: context-routed note missing 'space: $SLUG'"; exit 1; }
...
[ "$want" = "$have" ] || { echo "FAIL: context-note id parity want=$want have=$have"; exit 1; }
bash "$AS" clear >/dev/null 2>&1
echo "PASS context-env-routes-writes"
