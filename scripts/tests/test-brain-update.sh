...
cat > "$AGENTBRAIN_DIR/vault/update/config.json" <<'JSON'
...
mkcfg() { python3 -c "import json;p='$AGENTBRAIN_DIR/vault/update/config.json';d=json.load(open(p));d['auto_update']='$1';json.dump(d,open(p,'w'))"; }
...
mkcfg off
rc=0; out="$(ses 2>&1)" || rc=$?
if [ "$rc" -eq 0 ] && [ "$(git -C "$W2" rev-parse HEAD)" = "$before" ] && [ -z "$out" ]; then
	ok "session off -> silent no-op"
else
	bad "session off"; echo "rc=$rc '$out'"
fi

mkcfg notify
rc=0; out="$(ses 2>&1)" || rc=$?
if [ "$rc" -eq 0 ] && echo "$out" | grep -q "update available" && [ "$(git -C "$W2" rev-parse HEAD)" = "$before" ]; then
	ok "session notify -> reports, changes nothing, exit 0"
else
	bad "session notify"; echo "rc=$rc '$out'"
fi

mkcfg ask
rc=0; out="$(ses 2>&1)" || rc=$?
if [ "$rc" -eq 0 ] && echo "$out" | grep -q "Ask the user whether to update" && [ "$(git -C "$W2" rev-parse HEAD)" = "$before" ]; then
	ok "session ask (no TTY) -> hands off to the agent, changes nothing, exit 0"
else
	bad "session ask"; echo "rc=$rc '$out'"
fi

mkcfg auto
rc=0; ses >/dev/null 2>&1 || rc=$?
if [ "$rc" -eq 0 ] && [ "$(git -C "$W2" rev-parse HEAD)" != "$before" ]; then
	ok "session auto -> updates behind the gate, exit 0"
else
	bad "session auto"; echo "rc=$rc"
fi

# ── --session rate-limit: a fresh stamp suppresses the next run ──────────────
gq "$W2" reset --hard HEAD~1   # behind again so an update is available
mkcfg notify
date +%s > "$AGENTBRAIN_DIR/vault/update/.last-session-check"   # just checked
