...
# shellcheck disable=SC1091
# The helper prompts when stdin is a terminal. This test is about the non-TTY
# tiers; under the installer (which hands its children /dev/tty) it waited for
# Enter on two machines. Close stdin for good.
exec </dev/null
...
# shellcheck disable=SC1091
source scripts/tools/install-prerequisites.sh >/dev/null 2>&1 || true
