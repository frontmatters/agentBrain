...
mkdir -p "$SB/vault/learnings" "$SB/shared"
OLDID="$(bash "$ROOT/scripts/uuid5-gen.sh" "local/learnings/foo")"
printf -- '---\ndate: 2026-06-21\ntype: learning\ntags: [x]\nid: %s\n---\n\n# Foo\n' "$OLDID" > "$SB/vault/learnings/foo.md"

AGENTBRAIN_LOCAL_DIR="$SB/local" AGENTBRAIN_SHARED_DIR="$SB/shared" \
  bash "$ROOT/scripts/sync/promote-to-shared.sh" learnings/foo

[ -f "$SB/shared/learnings/foo.md" ] || { echo "FAIL: niet verplaatst"; exit 1; }
[ -f "$SB/vault/learnings/foo.md" ] && { echo "FAIL: origineel niet weg"; exit 1; }
