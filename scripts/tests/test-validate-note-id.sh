...
# test-validate-note-id.sh — unit tests for scripts/hooks/validate-note-id.sh.
...
VALIDATOR="$ROOT_DIR/scripts/hooks/validate-note-id.sh"
...
# Like run_case, but validates a CANDIDATE body against an intended (possibly
# not-yet-existing) target path via --content-file — the pre-write mode used by
# the Pi tool_call guard.
run_content_case() {
	local desc="$1" target="$2" body="$3" expected_exit="$4"
	local cand actual_exit=0
	cand="$(mktemp "${FIXTURE}/candXXXXXX")"
	printf '%s' "$body" > "$cand"
	bash "$VALIDATOR" "$target" --content-file "$cand" >/dev/null 2>&1 || actual_exit=$?
	rm -f "$cand"
	if [ "$actual_exit" = "$expected_exit" ]; then
		pass "$desc (exit=$actual_exit)"
	else
...
# exempt, like check-vault-content.sh. Must be no-op even with a non-uuid5 id.
...
# Case 11: --content-file, correct id, target dir does NOT exist yet (pre-write
# validation of a brand-new note). Proves the mode works before the file lands.
NEW_UUID="$(expected_uuid "local/projects/newproj/index")"
run_content_case "content-file correct id on non-existent target passes" \
	"$FIXTURE/local/projects/newproj/index.md" \
	"$(printf -- '---\ntype: project\ntags: [test]\nid: %s\n---\n# New\n' "$NEW_UUID")" 0

# Case 12: --content-file, wrong id → fails (the fabricated-id fingerprint).
run_content_case "content-file wrong id fails" \
	"$FIXTURE/local/projects/newproj/index.md" \
	"$(printf -- '---\ntype: project\ntags: [test]\nid: 00000000-0000-0000-0000-000000000000\n---\n# New\n')" 1

# Case 13: --content-file honours the exempt-path list too (exempt wins over a
# wrong id), so pre-write and post-write agree.
run_content_case "content-file exempt path is no-op" \
	"$FIXTURE/local/sessions/archive/2026-05-24-x.md" \
	"$(printf -- '---\nid: 00000000-0000-0000-0000-000000000000\n---\n')" 0
