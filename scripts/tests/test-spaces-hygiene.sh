...
# test-spaces-hygiene.sh — type:space paspoorten pass validate-note-id + check-vault-content.
#
...
SPACES_DIR="$ROOT_DIR/vault/spaces"
...
bash "$ROOT_DIR/scripts/checks/check-vault-content.sh" local/spaces >/dev/null 2>&1 \
	|| { echo "FAIL: check-vault-content errors on local/spaces"; exit 1; }
