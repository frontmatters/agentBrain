...
# check-local-content.sh must live at scripts/checks/ (its ROOT_DIR climbs two
...
# check-local-content reads the shared note-schema exemptions from here
...
cp "$ROOT_DIR/scripts/checks/check-local-content.sh" "$FIXTURE/scripts/checks/"
ln -s checks/check-local-content.sh "$FIXTURE/scripts/check-local-content.sh"
...
if [ -f "$FIXTURE/vault/findings/check-local-content.json" ]; then
	pass "findings JSON written"
else
	fail "findings JSON missing"
fi

if python3 -c "import json; json.load(open('$FIXTURE/vault/findings/check-local-content.json'))" 2>/dev/null; then
