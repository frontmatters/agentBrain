...
# (see scripts/checks/check-space-boundary.sh).
set -uo pipefail
...
mkdir -p "$FIXTURE/system/lib" \
         "$FIXTURE/vault/spaces/alpha/learnings" \
         "$FIXTURE/vault/spaces/beta" \
         "$FIXTURE/vault/learnings"
...
EOF
printf -- '---\ntype: space\nslug: alpha\n---\n' > "$FIXTURE/vault/spaces/alpha/index.md"
printf -- '---\ntype: space\nslug: beta\n---\n'  > "$FIXTURE/vault/spaces/beta/index.md"
...
expect "file under spaces/alpha" alpha \
	"$(bash -c 'source "$1"; infer_context "$2"' _ "$CTX" "$FIXTURE/vault/spaces/alpha/learnings/x.md")"

# 2) frontmatter space: on a file outside any space dir.
FM="$FIXTURE/vault/learnings/fm.md"; printf -- '---\nspace: beta\n---\n' > "$FM"
...
MIS="$FIXTURE/vault/spaces/alpha/learnings/mis.md"; printf -- '---\nspace: beta\n---\n' > "$MIS"
...
expect "explicit workdir replaces harness cwd" beta \
	"$(cd "$FIXTURE" && bash -c 'source "$1"; infer_context "" "$2"' _ "$CTX" "$CR_B/src/deep")"

# breukgeval: `cd` elsewhere must NOT switch context — file path dominates cwd.
expect "file path dominates cwd" beta \
	"$(cd "$CR_A" && bash -c 'source "$1"; infer_context "$2"' _ "$CTX" "$FIXTURE/vault/spaces/beta/y.md")"
...
rm -f "$FIXTURE/vault/.space-map.json"
