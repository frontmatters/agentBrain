...
# test-session-digest.sh — unit tests for scripts/flow/session-digest.sh.
...
run_digest() {
	...
	bash "$ROOT_DIR/scripts/flow/session-digest.sh" --format tsv "$@"
}
...
OUT_MD="$(CLAUDE_PROJECTS_DIR="$FIXTURE/claude-projects" PI_SESSIONS_DIR="$FIXTURE/pi-sessions" bash "$ROOT_DIR/scripts/flow/session-digest.sh" --format md 2>/dev/null)"
