...
F="$ROOT_DIR/vault/spaces/$SLUG/index.md"
...
bash "$ROOT_DIR/scripts/checks/check-local-content.sh" "local/spaces/$SLUG" >/dev/null 2>&1 \
...
if bash "$ROOT_DIR/scripts/new-space.sh" "__nsx__" --relation client >/dev/null 2>&1; then
	echo "FAIL: accepted missing --owner"; rm -rf "$ROOT_DIR/vault/spaces/__nsx__"; exit 1
fi
if bash "$ROOT_DIR/scripts/new-space.sh" "__nsx__" --owner X >/dev/null 2>&1; then
	echo "FAIL: accepted missing --relation"; rm -rf "$ROOT_DIR/vault/spaces/__nsx__"; exit 1
fi
