...
for name, f in d["fields"].items():
    assert f.get("type") in ("choice", "multi", "text"), f"{name}: bad type"
    if f["type"] in ("choice", "multi"):
        opts = f.get("options")
        assert isinstance(opts, list) and len(opts) >= 2, f"{name}: needs >=2 options"
        # Open-world lists (multi, or detected values like OS/editor) must keep
        # the always-present custom escape: the user can type their own answer.
        # Deliberately closed sets (language, channel, updateMode) stay closed.
        if f["type"] == "multi" or f.get("detect"):
            assert any(o.get("custom") for o in opts), f"{name}: missing custom escape"
...
if grep -q 'test-onboard-choices.sh' "$ROOT_DIR/scripts/checks/doctor.sh"; then
...
FLOWSPEC="$ROOT_DIR/vault/tools/onboard-proto/flow-spec.json"
if [ -f "$FLOWSPEC" ]; then
...
# 8) profiles: every preselect key is a real field, every preselect value a
#    real option of that field; exactly one full-flow escape ("no profile")
if python3 - "$CHOICES" <<'PY'
import json, sys
d = json.load(open(sys.argv[1]))
profs = d.get("profiles", {}).get("options", [])
assert len(profs) >= 3, "profiles missing or too few"
fields = d["fields"]
for p in profs:
    assert p.get("value") and p.get("desc"), "profile option needs value+desc"
    for key, vals in p.get("preselect", {}).items():
        assert key in fields, f"preselect targets unknown field {key}"
        known = [o["value"] for o in fields[key]["options"]]
        for v in vals:
            assert v in known, f"preselect {key}={v} is not a real option"
assert sum(1 for p in profs if p.get("full")) == 1, "exactly one full-flow escape expected"
# apply-mode rides along on every channel option (merged updates question)
assert all(o.get("apply") in ("ask", "notify", "auto", "off")
           for o in fields["channel"]["options"]), "channel options need apply keys"
PY
then pass "profiles preselect values are real options + apply keys present"; else fail "profiles section invalid"; fi

# 9) SKILL.md landed the language redesign
