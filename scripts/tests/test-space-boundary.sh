...
# The breach case stages and unstages a path in a vault index. Never the real
# one: two doctors at once (a deploy's and a push's) plus the vault's own
# background push all contend for its .git/index.lock, and the test then fails
# on a lock, not on the guard. A throwaway clone of the real vault (shared
# objects, so it is quick) gives the guard a real repo with the real layout.
TMP_VAULT="$(mktemp -d)"
if git -C "$LOCAL_DIR" rev-parse --git-dir >/dev/null 2>&1; then
	git clone -q --shared --no-checkout "$LOCAL_DIR" "$TMP_VAULT/vault" 2>/dev/null \
		&& git -C "$TMP_VAULT/vault" checkout -q -- . 2>/dev/null \
		|| { rm -rf "$TMP_VAULT/vault"; git init -q "$TMP_VAULT/vault"; }
else
	git init -q "$TMP_VAULT/vault"
fi
LOCAL_DIR="$TMP_VAULT/vault"
...
bash "$GUARD" >/dev/null 2>&1 && { echo "  FAIL[display]: a space kept its display name" >&2; rm -rf "$_d"; exit 1; }
rm -rf "$_d"
echo "  ok[display]: a display name on a space is refused"
# confidential: false is not a state a space can be in
_d="$LOCAL_DIR/spaces/zz-test-public"
mkdir -p "$_d"
printf -- '---\ntype: space\nslug: zz-test-public\nrelation: research\nconfidential: false\n---\n' > "$_d/index.md"
bash "$GUARD" >/dev/null 2>&1 && { echo "  FAIL[non-confidential]: a space with confidential: false passed" >&2; rm -rf "$_d"; exit 1; }
rm -rf "$_d"
echo "  ok[non-confidential]: confidential: false on a space is refused"
rm -rf "$_d"
