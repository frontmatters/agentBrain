...
set -uo pipefail
# Dependency guard: brain-extract needs yq. Skip VISIBLY when absent (fresh
# machine before brew bundle) — never fail with a cryptic downstream message.
if ! command -v yq >/dev/null 2>&1; then
	echo "SKIP: 'yq' not installed — brain-extract packaging needs it (brew install yq)" >&2
	exit 0
fi
ROOT_DIR="$(cd "$(dirname "$(realpath "${BASH_SOURCE[0]}")")/../.." && pwd)"
SLUG="__extest__"; SP="$ROOT_DIR/vault/spaces/$SLUG"
...
bash "$ROOT_DIR/system/skills/brain-extract/bin/brain-extract" --space "$SLUG" --out "$OUT" --vault "$ROOT_DIR/vault" >/dev/null 2>&1
