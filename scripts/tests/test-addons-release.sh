...
ROOT="$(cd -P "$(dirname "$(realpath "${BASH_SOURCE[0]}")")/../.." && pwd -P)"   # realpath: the scripts/ shim is a symlink
...
# youtube-digest: 67 is the measured LINE coverage on 2026-09-06 (bun's table is
# "% Funcs | % Lines"; the gate reads the lines column). It was 35 on v1.10.1,
# with two failing tests. The 75 that stood here was never reached, so the gate
# had been red since it was written and taught nobody anything. A ratchet: raise
# it when the number rises, never lower it.
run_coverage "youtube-digest" "system/addons/youtube-digest" 67
