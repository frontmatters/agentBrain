...
TPL="$ROOT_DIR/templates/vault/preferences/personal/identity.md"
SETUP="$ROOT_DIR/scripts/setup/setup-templates.sh"
PASS=0; FAIL=0
...
# 5) setup-templates.sh auto-seeds it from templates/vault/ (mirrors local/)
if grep -q 'templates/vault' "$SETUP"; then pass "setup-templates seeds from templates/vault (auto-seed)"; else fail "setup-templates does not seed from templates/vault"; fi

# 6) doctor runs the identity test
if grep -q 'test-onboard-identity.sh' "$ROOT_DIR/scripts/checks/doctor.sh"; then
