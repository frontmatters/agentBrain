...
# Registry addon ids (system/addons/clients.md): their skill wrappers are pruned
# from release artifacts on purpose — a listed-but-absent registry skill is not
# drift there, just a registry-distributed addon.
# Both sources are optional (repo checkout has no REGISTRY-ADDONS.txt; a heavily
# pruned artifact could lack rows) — never let a missing source kill the check
# silently under set -e.
registry_ids="$( { grep -E '^\| [a-z0-9-]+ \|' system/addons/clients.md 2>/dev/null | sed -E 's#^\| ([a-z0-9-]+) .*#\1#' || true; cat system/addons/REGISTRY-ADDONS.txt 2>/dev/null || true; } | sort -u)"

# Direction 2: every index row has a skill directory.
while IFS= read -r name; do
	[ -n "$name" ] || continue
	if ! grep -qx "$name" <<<"$dir_names"; then
		if grep -qx "$name" <<<"$registry_ids"; then
			echo "note: \`/$name\` listed but absent here — registry addon (pruned in release artifacts)"
			continue
		fi
