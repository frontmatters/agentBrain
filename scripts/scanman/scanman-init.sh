...
for filename, artifact in artifact_map.items():
    ...
    rel = f'vault/research/repo-distill/{slug}/{stem}'
    uid = make_uuid(rel)
...
text = text.replace('- Next action', '- Next action: run `bash scripts/scanman/scanman-scan.sh <repo-path> {}` and then manually enrich the generated docs'.format(slug) if repo_path else '- Next action: populate `00-file-inventory.md`')
...
if [[ -n "$REPO_PATH" ]]; then
  echo "Next: bash scripts/scanman/scanman-scan.sh '$REPO_PATH' '$REPO_SLUG'"
else
  echo "Next: run scripts/scanman/scanman-scan.sh with a repo path or fill files manually"
fi
