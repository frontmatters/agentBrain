...
usage() {
  echo "Usage: bash scripts/scanman/scanman-complete.sh <repo-path> [repo-slug] [goal...]" >&2
}
...
if [[ ! -d "$TARGET_DIR" ]]; then
  bash "$AGENTBRAIN_DIR/scripts/scanman/scanman-init.sh" "$REPO_SLUG" "$REPO_PATH" "$GOAL"
fi

bash "$AGENTBRAIN_DIR/scripts/scanman/scanman-refresh.sh" "$REPO_PATH" "$REPO_SLUG"
