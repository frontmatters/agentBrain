...
if [ -f brain.json ] && [ -d vault ]; then
	...
	if [ -n "$ns" ]; then
		if [ ! -s vault/brain-namespace.backup ]; then
			printf '%s\n' "$ns" > vault/brain-namespace.backup
			echo "fix: wrote vault/brain-namespace.backup (UUID5 namespace backed up in the private repo)"
			fixed=$((fixed + 1))
		elif [ "$(cat vault/brain-namespace.backup)" != "$ns" ]; then
			# Write-once: a mismatch is the event the backup exists to expose, not to paper over.
			echo "fix: brain.json namespace differs from vault/brain-namespace.backup; NOT overwriting. check-vault-content tells which one the vault's ids use; fix brain.json or the backup by hand." >&2
		fi
	else
...
if [ -x scripts/setup/setup-skills.sh ] || [ -f scripts/setup/setup-skills.sh ]; then
	if bash scripts/checks/check-skill-links.sh >/dev/null 2>&1; then
		: # in sync — nothing to do
	else
		bash scripts/setup/setup-skills.sh >/dev/null 2>&1 || true
...
for d in vault/findings vault/sessions vault/logs vault/backlog vault/reviews vault/addons; do
	if [ ! -d "$d" ]; then
		mkdir -p "$d"
