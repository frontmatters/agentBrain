...
if [ -f brain.json ] && [ -d vault ]; then
	...
	if [ -n "$ns" ]; then
		if [ ! -s vault/brain-namespace.backup ]; then
			...
			echo "fix: brain.json namespace differs from vault/brain-namespace.backup; NOT overwriting. check-local-content tells which one the vault's ids use; fix brain.json or the backup by hand." >&2
		fi
	else
