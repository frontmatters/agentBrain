...
# The factory is where agentBrain is built, never where it is installed. An
# installer run over one of its checkouts resets that checkout to the published
# snapshot, re-points its origin and prunes the shared skills; it did, twice, on
# 2026-09-06, through the ~/Developer/agentBrain proxy link. A factory checkout
# is recognised by the factory.json beside it, so resolve links before looking.
_ab_refuse_factory() {
  local dest="$1" real
  real="$(cd -P "$dest" 2>/dev/null && pwd -P)" || return 0   # nothing there yet: a fresh install
  [ -f "$real/../factory.json" ] || return 0
  printf '\n  install.sh refuses to install over a factory checkout:\n    %s\n    resolves to %s\n  A factory checkout is switched, not installed: brain use dev|live\n\n' "$dest" "$real" >&2
  return 3
}
_ab_refuse_factory "$DEST" || exit $?
...
# The rest of the context a remote reader needs to reproduce a run: where the
# shell stood (the old banner read a VERSION from there), where temp files go,
# and which home the install targets.
ln_ "  ${D}from: $(pwd) · tmp: ${TMPDIR:-/tmp} · home: ${AGENTBRAIN_HOME:-$HOME}${N}"
ln_ "  ${D}with: $(bash --version | head -1 | sed -E 's/GNU bash, version ([0-9.]+).*/bash \1/') · git $(git --version 2>/dev/null | awk '{print $3}') · $(uname -sm)${N}"
printf '\n'; slp 0.15
