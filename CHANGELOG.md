...
- **Doctor guard: session-start update-check must stay quiet** — a new
  ...
  private/LAN origin URL) and never hangs on an unreachable origin. Guards the
  v1.10.0 regression from recurring: it fails the doctor before a re-break ships.
...
- **Unified explainer index** (`scripts/build-explainer-index.py`) — one
...
- **Ecosystem map** (`scripts/build-ecosystem-index.py`) — a searchable index of
