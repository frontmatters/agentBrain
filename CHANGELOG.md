...
- `decision-page`: tweede template `template-register.html` + `register.js` voor registerpagina's (tien tot twintig kleine besluiten uit een gaplog of review): herkomst per bevinding (vandaag gemeten / geciteerd), correctieblok, meerregelige toelichting per besluit, drie-standen themaschakelaar. Themaregel vastgelegd: merk van de klant indien aanwezig, anders agentBrain-lime (#bef264), niet te verwarren met Frontmatters-oranje.
### Added
- **`rename-symbol`: a rename follows the symbol, not the file.** Find-and-replace over a codebase changes three things that only look alike: code, the copy a person reads, and the record of what something *used* to be called. Rewrite the second and a user-visible sentence goes wrong in a well-formed way; rewrite the third and a decision record starts claiming a judgement was made on a name that did not exist yet. Neither fails a build, a test, or a diff review. The skill splits every hit into those three categories (`sweep.sh`), sizes the replacement to how short the name is (under five characters is by hand, because a three-letter name sits inside other words), and verifies by running and looking: full suite rather than the tests you think are affected, rendered pages read for words from the other language, shadow roots walked, and a positive control so that "the bad string is absent" cannot be confused with "I measured nothing". A screen that could not be reached is named as a gap, not counted as a pass.

- **`scan-local-references.sh`: where does `local/` still live?** The vault moved to `vault/` and the directory is gone from every checkout, yet roughly 630 path references remain per codebase. The script sweeps the factory read-only and splits them by consequence rather than counting them: a reference under `scripts/` or `system/` is executed, so a dead path there costs a command or a body of knowledge, while one in a changelog or a session log describes the layout of its time and rewriting it would falsify the record. What the split makes visible is that a global rename is not available: every UUID in the brain is computed over the `local/` spelling, `uuid5-gen.sh` folds `vault/` back to it, and a test exists to prove that parity. So the leftovers are three different things wearing one word, and the way out is a decision (migrate the id base once, or keep `local/` as the internal spelling) rather than another sweep.

- **`check-agent-pointers`: do the paths each agent is handed still exist?** Every agent gets a pointer block naming what to read at session start, and the setup scripts append that block and then skip when one is already there. So they verify that a block EXISTS, never that it still POINTS somewhere. When `vault/` replaced `local/`, one agent's block was rewritten and another's was not, and nobody noticed, because a missing file is silent: no error, no warning, simply no knowledge arriving. The check found six dead pointers in the Copilot block and one in Claude's on the day it was written. Absence stays allowed where the instruction says so, since a line reading "any existing files under" describes an optional directory rather than a broken pointer.
- **Reading a repository's own `AGENTS.md` is now a shared instruction**, in `system/agent-config/shared.md` rather than in one agent's personal config, so all three agents get it. A repository carrying that file has written down how to work in it, and an operation touching a credential, a registry, a deploy or a release always has such a procedure: find it before the first attempt, and not finding one is itself the finding. Written after a publish failed 401 while the repository's `AGENTS.md`, two directories above the cursor, carried both the procedure and the warning that the key chosen gives exactly that 401.

- **A gaplog is now a first-class note type, next to the backlog.** A backlog collects what we intend to build; a gaplog collects what already looks built and does not do what it promises. They are not two flavours of one list. A backlog item is a promise not yet made, so deferring it is free and it needs no evidence. A gaplog item is a promise already made and broken, so it was prioritised the moment the promise was made, and the assertion that something is broken right now has to be provable. That difference is why `new-note.sh gaplog` scaffolds a register rather than an essay: every row carries where it plays, who found it, evidence (a file and line, a measurement, or a query) and a verification date, plus the split by who can close it: agent, owner, or outside. The columns are the contract, and they were not designed but earned, because over one session an owner tightened the same document four times and each correction added exactly one of them. `/gaplog` carries the routing test for the border cases, namely that a gap is a broken promise to a user while missing code is backlog, and the rule that keeps the two registers from drifting into each other: a gap may spawn a backlog item, never the reverse.

- **A learning about a trap has to say how it is enforced.** On 2026-09-15 an agent wrote a learning about a bash working-directory pitfall and then walked into that same pitfall four more times within hours, twice after writing it. A second learning, about backticks inside a `css` template, fared no better: three times. The knowledge sat in the vault and changed nothing, for three reasons. Retrieval needs a trigger, and these mistakes give none (a wrong working directory reads as "that file does not exist"). The failure happens at typing time, not thinking time, where no document can reach. And recall is probabilistic, while enforcement is not. `check-enforcement.sh` therefore splits its response by how certain it can be. A learning that *claims* `enforcement: hook|gate` must name it in `enforced_by:`, and that target must exist; a claimed guard that is gone is worse than none, because you stop watching for the mistake. Trap-tagged learnings with no declaration at all sit under a ratchet that may only fall, the same mechanism the English ratchet uses, so the backlog shrinks without blocking work today. Baseline: 28 of 30.
- **`claude-bash-path-guard.py`**, a `PreToolUse` guard on `Bash`. It blocks a read command handed a relative path that does not exist from the current working directory. The Bash tool keeps its working directory between calls, so one `cd` silently changes what every later relative path means, and the result fails as "No such file or directory", which reads as "the file is missing" rather than "you are standing somewhere else". Deliberately narrow, because a guard that cries wolf gets switched off: read commands only, redirects excluded, and the first positional of `grep`, `rg`, `sed` and `awk` skipped, since that is a pattern and patterns contain slashes all the time. It caught its author twice within minutes of being installed, and produced one false positive on a redirect target, which is now a test case.
